...
export function validateReleaseSource({ sha, mainSha, developSha, tag, version, release, environment }) {
  ...
  if (!['1.0.3', '1.0.4'].includes(version) && !environment?.protection_rules?.some(rule =>
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  if (isFormalRelease({ eventName, refType, refName })) {
    ...
    validateReleaseSource({
      ...
      environment: ['1.0.3', '1.0.4'].includes(version) ? null : optionalGitHubResource(`repos/${repository}/environments/stable-release`),
    });
  }
