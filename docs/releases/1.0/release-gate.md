...
2026-09-07 MOBI6/KF8增量只计真实已观察子项，详见evidence；短样一屏显示不代替精确跨页锚点/冷恢复时限，源码/开发APK一致性与最终冻结RC分别记录。本轮无范围/阈值变更。

2026-09-07展示验收增量：SYNC-04/AUDIO-14真实原场景闭环，保持“成功服务端读取为展示依据，仅未同步本地位置可作持久覆盖”的既有语义；Chrome离线仅为制造跨端旧缓存对照，不新增首页特殊业务。原生冷首页、详情及播放器恢复分别验证，音轨列表无需因本地进度变化重新读取静态内容。无阈值/发布范围改变，具体PASS及局限见release-evidence，仍须最终同冻结RC验证。

2026-09-07 POS-07按已冻结最后事务提交语义执行：Chrome离线首次C在Android N之后到达，完整C成为新current且仅新增一次revision；此可见回退不能被“取最大进度”消除。实际恢复正确与首页/详情投影正确分开验收；本轮另发现原生已确认旧缓存覆盖服务端首页、Web离线列表错误重连后未恢复，须独立修复，不由POS-07协议通过抵销。

2026-09-07 POS-05真实丢ACK链补齐：复用现有production fixture、Chrome原生CDP响应拦截及Android普通播放器，在M已提交后丢弃实际响应，另端提交N，再放行Chrome原body重试；分别验证原receipt、当前完整N、不增revision和异payload冲突。仅该MP3客户端组合通过；人工持有响应不计5秒时限，未直接读取IDB不声称pending清空。无业务/测试辅助改动，不外推其他引擎、iOS或最终冻结RC。

2026-09-07 STATUS-01按原POS-11语义闭环：书级/资源级状态消费既有completed owner，进度投影只改进度；新包普通M4B标记/取消、强停重开和原位置实际恢复通过。针对性100%反例与真实67%场景分别留证；无验收阈值/范围改变，不外推其余格式或最终冻结RC。

2026-09-07 POS-11执行边界：已读状态以现有服务端completed事实为准，原生本地进度展示覆盖不得以百分比重算该状态；67%可标已读，取消已读不清除实际位置，100%位置本身不能代替独立状态。真实详情标记后仍“在读”的STATUS-01须按原场景和相邻回归闭环，不改既有产品语义或阈值。

2026-09-07 重启与离线证据边界：既有API/Worker模块可直接在同一专用数据目录按实际进程归属终止后重新启动，用实际Android确认及恢复验收，不必扩展fixture控制框架。原生SQLite pending、服务端receipt和新引擎capture必须分开判定；latest-only新捕获可替代旧pending，但不能把未观察到原mutation的场景标为原body精确重试通过。隐藏的详情预览不因验收自行开放。

2026-09-07 AUDIO-13原失败已闭环：同原件新开发包的播放中与暂停状态旋转保持会话，普通注销仍停止。满足上述生命周期缺陷的原门禁验收边界，不将其外推为全部格式/进度异常或同冻结RC放行；停止本缺陷工具扩展。

2026-09-07 AUDIO-13执行要求：移动音频配置重建不得被Shell销毁错误终止，账号/namespace退出仍必须停止。原旋转中断、候选自动检查及新包真实旋转/注销相邻分别留证；持久位置可恢复不抵销活动会话中断。不增加平台或测试框架，仍使用原门禁与隔离设备入口。

2026-09-07 POS-10增量：MP3活跃暂停会话通过Chrome实际断网/重连与Android既有任务ON_RESUME验证远端提示，不自动seek；仅显式跳转采用远端位置。保持原语义、阈值与各引擎要求，服务受控重启/重开分项仍待执行。本轮未改业务或测试辅助代码。

2026-09-07 POS-06执行边界补充：已按原语义实际验证Android同步owner收到迟到ACK时完整新pending不丢，并在重建owner/数据库后用原mutation真实重试确认。延迟控制点在实际HTTP成功后的测试port返回处；不能将此证据称为socket丢包、普通UI竞态、Chrome或进程强杀通过。原5秒确认期限及完整位置/身份断言保持，必要相邻通过后停止本辅助改动；其余异常与同RC要求不变。

2026-09-07 SYNC-03执行结果覆盖：ead1ba20真实Chrome重载详情与随后播放器恢复均已通过，原生详情相邻及实际presentation同源亦核验，具体证据与清理见release-evidence。此为原门禁下的具体缺陷闭环，不变更阈值/范围，也不替代其他异常或同冻结RC完整验收。正式APK/IPA仍按负责人决定暂缓，未作正式发布。

2026-09-07 SYNC-03真实详情显示失败单独收敛：fresh reload使用服务端新投影，仅本地pending及本页面真实捕获可覆盖；不读取已确认旧缓存覆盖远端新确认，不通过百分比重造Locator。正常恢复与详情正确性分别验收。

2026-09-07 跨端音频增量：M4B/M4A/AAC 普通 Chrome↔Android 交接按真实源端确认、同 resource/asset 与目标实际媒体时钟验收，三格式双向短时子项通过。详情展示与播放器恢复分别判定；本轮重载详情暂显旧位置、打开播放器后正确刷新，前者待定位，不由恢复通过抵销。没有更改工具、格式或阈值；不外推离线、后台、长时和最终冻结 RC。

2026-09-07 AUDIO-11/12原失败、映射/路由/提示针对性与相邻自动回归、独立审查及新包真机普通UI已闭环，停止相关工具完善。原生音频位置验收以同资源/asset的服务器确认与系统播放器首次Playing值对应；强停独立包后从首页实际进入，不能只验证详情或runtime直调。M4B/M4A章节短样分别按实际源路径留证，不外推完整长时和异常矩阵。

2026-09-07普通音频入口验收补充：首页继续、作品详情继续及播放器恢复分别沿实际路径验证。某入口误将音频交给电子阅读器时，其他入口恢复通过不能抵销该失败；必须复用现有能力路由/音频启动owner修复并复验，不改Locator或格式承诺。原始失败、对照成功和新候选证据分别追溯。
...
文档版本：v1-R2（逐项验收与缺陷收敛；既有R1证据编号保留；尚未冻结RC）
建立日期：2026-09-06  
...
- DEC-10（2026-09-07，负责人调整验证成本）：局部修复默认只验证原失败、实际受影响边界和必要模块检查；同代码/依赖/环境结果复用。仅影响扩散、无法限定的公共基础变更、既定集成检查点、最终冻结RC或用户要求才逐级扩大。达到原失败与关键相邻通过、范围明确且无具体未解风险时停止；不强制小补丁独立工作树、多轮审查或全量回归，不新增通用框架。记录集中在现有evidence，其他台账只保留状态/引用；最终门禁不变。

- AUDIO-10已按原失败、针对性与相邻自动回归、真实Chrome原场景完成闭环，停止该缺陷工具完善。四轨短时顺序/切换/恢复/首轨自动接轨分别通过，不外推长时、全部边界、原生或冻结RC；最终候选仍须统一源码回归。

- 多轨详情与播放器必须消费一致的音轨顺序；音频详情复用现有导入持久化sequence_index，不以文件名再次重排覆盖该结果。章节样本短时通过、四轨排序原失败及候选回归分别登记，未完成实际回归前不得关闭缺陷。

- 普通界面音频验收须区分实际播放器时钟、服务端确认位置和样本身份。M4B/M4A即便字节相同，也分别实际导入和操作；该结果不代替章节/多轨。章节样本可用原ffmpeg以-c copy加入测试元数据并保留来源/hash，通过现有导入/播放器验收；只准备输入不计客户端通过。本轮短时普通界面已达停止条件，后续只补未测场景和最终RC回归。

- 音频后台与中断验收分别记录实际生命周期、系统动作和确认持久化。MP3 STOPPED后台及真实短暂音频焦点中断已有开发子项证据，不外推锁屏/来电或四格式全矩阵。新增隔离前置只在fixture拥有播放前使用全局活跃状态，重开不得把自己的媒体服务误判为外来会话；原App会话、位置和期限断言保留。已完成原失败及相邻回归后停止扩工具，最终RC仍需按实际冻结源码回归。

- DEC-09（2026-09-07，负责人收敛有声书验收范围）：本轮仅验证明确列出的M4B、MP3、AAC、M4A四种源格式；“三种”的数量描述按实际四项名单执行。其他音频格式及额外编码组合不再要求逐项验证，也不再因缺样本/解码验收阻塞本轮。此决定覆盖下文和历史台账中的全编码矩阵要求；仅改变验收范围，不删除产品格式接收能力、不修改安全契约、不将未测项填PASS。四种格式的实际容器/编码仍须准确留证，不能以M4A替代M4B；既有后台/中断、长时/多轨、进度/异常与最终RC要求保持。已完成的范围外证据及原失败保留，不继续扩展或重跑。

- Android逐编码短时验收优先复用已核验安装包及现有入口。固定fixture样本之外的真实文件可在全新库初始化前加入专用目录，保留最初生成清单、显式记录新增来源/hash与准确样本数，再运行原校验；不隐瞒人工输入准备、不修改运行预期/服务响应或扩展通用工具。M4A/AAC-LC及Ogg/Opus两子项已达到停止条件，其他组合单独验收。

- 漫画详情指定页入口在现有URL消费语义下转换展示页号至零基索引，PDF入口保持一基；不全局重定义Locator或页码契约。受控链接函数复现与真实页面恢复分别记账，后者缺失仍待验收。

- READER-05/06控制栏缺陷按既有语义收敛：PDF“上一页/下一页”复用引擎分页命令，漫画展示页号与零基命令索引在边界转换；不生成虚假目录、不改保存协议。真实原失败、候选代码检查和浏览器执行受阻分别记账，工具审批拒绝不是测试通过或产品失败。

- READER-04实际PDF启动失败按现有v5生成契约修复：Web主资产选择使用role=PRIMARY，由bootstrap owner校验后供引擎复用；旧mock字段纠正并保留负例/安全/流式断言。当前仅代码回归通过，真实PDF及漫画相邻完成前不关单；不新增工具框架或分支，不更改格式承诺和安全策略。

- POS-03按实际恢复前提登记：`7e207c38`只通过Chrome桌面/移动视口已确认MP3暂停后强杀并正常重登录恢复，登录前完整IDB持久性、既有401私有缓存清理和服务端确认位置恢复分别举证。该测试不要求绕过认证或保留应清理的私有缓存；2秒恢复阈值不变，未确认/其他引擎/原生场景不由此覆盖。原错误测试预期及失败保留，原场景和必要相邻已通过，停止工具扩展。

- 分支收敛：用户提出减少分支后，后续默认在`codex/release-1.0-convergence`串行修改、按逻辑提交推送；子代理优先只读审查和复用既有工作区独立验证，仅确有并行写入/版本对照需求时再隔离。已验收子提交逐项审查整合，不在最后批量合并全部分支；现有未整合工作、原失败证据和用户改动均保留，不自动删除工作区或分支。

- POS-04离线验收区分页面重建、进程异常退出和另端交接。`02d6ea2d`的Chrome桌面/移动视口EPUB已实际证明离线完整pending经页面关闭重建保留，并在重连5秒内完成原mutation确认、本地持久化及独立GET；仅对应子项PASS，不覆盖POS-03强杀或原生离线。针对性用例与原相邻已通过，按DEC-07停止该项工具完善；其余场景继续沿现有入口收敛。

- 用户已明确允许SYNC-02的127.0.0.1:3107/3108两份隔离production测试服务；随后自动审批仍拒绝同一启动命令，该工具限制不重新解释为缺少用户授权。对应浏览器用例仍NOT_RUN，等待手动启动，其他可执行门禁继续。

- 连续播放时长以实际已记录的观察窗口判定，不能在最后一次网络确认跨过截止时刻后直接停止并丢弃末段采样。现有工具只修正此终止条件；实际用例通过、媒体末点与完整采样窗口分别记账，最后的完整窗口在后续候选回归补齐，不降低时长或停顿阈值。

- DEC-08（负责人接管OPDS客户端验收）：用户明确“OPDS 客户端标记为通过，我会自行测试”。OPDS-01/02及OPDS-03真实客户端部分记为PASS（负责人放行，用户自行测试），不再阻塞本轮收敛；这是负责人验收安排，不是代理已完成双客户端实测的证据。停止客户端安装、权限及工具准备。协议安全、原文件与Range自动回归仍保留；第三方进度同步仍按DEC-05不支持。

- DEC-07（测试基础设施收敛）：从本次补充起默认冻结辅助工具的功能扩展，优先现有入口与可追溯实际操作。仅有效Gate/Case无法执行或判定、工具导致误判/遗漏失败/数据或凭据安全/清理问题、具体未关闭问题必需的最小观测允许改动；每次在现有台账注明服务项、现有方法不足和停止条件。针对性回归保留；不得为未来复用、风格、配置/平台扩展或报告美化增加框架。原失败、必要相邻回归和真实验证通过即停止工具完善，除新影响或最终RC回归外不反复扩展已关闭项。缺外部条件只阻塞相关事项，不用工具开发代替真实验收；安全、持久化和竞态自动回归不由人工观察替代。汇报以真实缺陷关闭、必测场景和剩余阻塞为主，测试/脚本/提交数量不代表发布完成度。

- 持续播放验证须由实际媒体引擎确认样本剩余时长，不以编码命令的目标时长、UI滑块值或短片循环代替。可用公开语料的已记录长文件派生样本做连续引擎测试，需注明内容重复的样本限制。跳转、暂停及重开的位置验证保持原误差阈值；局部连续播放通过不能覆盖随后恢复失败。

- DEC-06（用户最新范围调整）：超大书库验证保留为可独立执行的脚本，不再要求 AI 在本次收敛中持续执行大规模压力测试。当前导入性能采用已有安静 1 万规模真实导入/重扫证据验收，该范围已通过。10 万/30 万规模、完整长时和三端混合压力场景转为后续按需验证，不再阻塞本轮代码收敛。下文 RG-05 的扩展规模/时长清单保留为脚本验证目标，不再是本轮必测条件；不得由此宣称 30 万规模或低功耗 NAS 已通过。原性能阈值、数据完整性和其他门禁的功能/播放稳定性要求不变；最终 RC 的证据追溯规则仍适用。
