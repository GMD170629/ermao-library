...
| IMP-02 / RG-02 | P2；中英名、空格、特殊字符、多层、图片/音频目录 | 导入后进入每层，切换排序/分页，打开图片目录与多轨音频 | 不乱码、不漏资源、不用资源数决定页面类型；正常播放/阅读 | 66446a45/imp02-special-paths-20260907/ | PARTIAL：Chrome特殊路径/多层TXT、24本排序分页、图片目录解码及两轨MP3入口通过；图片目录标题IMPORT-02 OPEN、MEDIA-02封面500重开；其他平台/复杂布局另计 |
