...
| COM-DIR | DIR-N/C：实际图片目录，多层/自然排序/非图片/已有图片类型 | W/A/I；P2 | F；图片文件数与Resource/Book数分开 | DIR-X：损坏图、单页失败隔离 | PARTIAL：bbb3fd71 Android普通IMAGE_DIR六张PNG/JPEG顺序/末页边界、README非图片过滤、确认后冷首页恢复第5页实际图像PASS；完整位置/原件及最终新pending限制见evidence。复杂/坏页、W/I及最终RC仍待 |
