...
- `POST /api/automation/grants/{id}/reveal` 仅接受本人有效 Cookie 会话，拒绝过期／撤销凭证，响应 no-store，审计只记身份与授权 ID。自动化管理接口不额外要求或比较 Origin，沿用 Cookie、角色与资源归属校验。列表不暴露明文；浏览器仅在当前授权配置面板内保留明文，关闭、切换授权／分区／账户即丢弃。
- HTTP 和 HTTPS 均支持域名、IP、端口和实际部署前缀，不再有额外 HTTP 选项；合法 URL、Bearer、MCP 传输的 Origin/Host 校验保留，入口为实际根地址的 `/api/mcp`。
