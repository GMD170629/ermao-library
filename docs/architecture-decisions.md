...
- 同库存在排队、运行或失败未恢复的扫描时，资源／识别任务等待，其他书库可以继续。新的扫描请求合并未完成范围，复用或合并其队列记录；完整枚举才允许按原缺失策略清理。应用层 `ContinueImportTask(force=True)` 显式清除该资源资产的成功处理版本并复用同一待办，保留可读结果和身份；默认继续导入仍增量复用。
...
- v1.1.0 发布运行 `35072969925` 在镜像构建成功后因多架构索引摘要冲突而打包失败。维护者于 2026-09-16 授权一次恢复：保留标签及原应用提交 `625b755039d76e76095fa8e1a4da531e4e4ef075`，复用已验证镜像 `sha256:53df4e06367f5b75ff24ba19fa5343a8415c979e44aec094d2f224d6c9be695a`，按平台子摘要生成更新包并补齐 FPK；本地与远端摘要核对通过后恢复发布，不重建镜像。打包脚本修复另行同步 main/develop，允许这两个分支领先该标签；仅适用于本次恢复，不改变其他版本门禁。
