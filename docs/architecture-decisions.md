...
- 首次、补齐与用户重试共用 ContinueImport 和 `LibraryImportTask` 单消费者队列；生产任务为 SCAN_LIBRARY、CONTINUE_SOURCE、IMPORT_BOOK。旧 IMPORT_RESOURCE、IDENTIFY_BOOK、IMPORT_ASSET 仅供升级映射与历史查询，不再由普通 Worker 领取；不增加第二套队列、租约、heartbeat 或自动接管。Book 行按 `bookId` 唯一，合并本书扫描范围和 Resource ID，执行版本与后续请求分别持久化；资源游标、阶段、有限退避和终态由该行拥有。解析与扫描在事务外，部分成功不因其他项失败回滚；终态持久化重试不重跑已提交的业务结果。
- 单文件资源、IMAGE_DIR 与 AUDIOBOOK_DIR 在所属 Book 阶段直接调用唯一资源步骤，不创建生产 Resource 子任务。单书超过 128 个 Resource 时按 Book ID 和 Resource ID 分页，每轮至多处理 128 个并保存游标后让出；处理中新增变化保留后续工作。音频目录复用目录资源执行和资产批次；只替换变化音轨自身章节，结束后统一执行音轨／章节安全重排、总时长和数量汇总。存在未知时长时总时长保持未知。内嵌封面以资源内内容哈希去重保留候选引用，最终只发布选中产物；0014 合并旧未完成音频任务。图片目录最多 200 张一个资产事务；节点校验和资产写入使用集合操作。目录元数据、自然顺序封面选择和资源数量在文件处理结束后统一收尾，不为每张图片复制目录候选。旧未完成图片任务由 0013 合并，已提交批次在中断恢复时复用。
...
- 每库最多一个 QUEUED 和一个 RUNNING 书库扫描；运行时新事件排入后续任务，不修改运行中范围。待执行范围合并，递归父目录覆盖子目录，全库覆盖局部。自动与手动共用对账和缺失清理规则；目录项在事务外按至多 200 项读取、核对和提交，完整枚举后才允许缺失清理。访问、属性读取、迭代或探测 I/O 失败保留对应目录数据，任务标记失败，其他成功目录不回滚；取消不记为成功。指定失败任务走 task-scoped ContinueImport。
- 扫描复用已确定的资源归属；变化节点与本书待办同事务提交，同一目录的一轮变化只登记一次资源处理请求。`scanSeenGeneration` 标记本轮确实观察到的直属节点，完整枚举后按页查找未见节点；`observedAt` 仅在观察属性变化时更新。`scanContextVersion` 保存 sidecar／指定封面属性和相关配置的扫描指纹，不代表资产成功处理版本，不全文哈希媒体。无变化扫描不重新 dirty Book 或执行资源步骤；元数据变化复用文件结果。根级完整批次可在继续发现前推进一个已确定 Book；尚未完整枚举的 Book 内部不得提前收尾。
- 依赖按必要输入和扫描范围判断，不以书库或创建来源判断：单文件 Book 的源节点观察已提交即可执行；目录 Book 等待覆盖其锚点的持久未完成扫描范围，带本书扫描工作的 Book 可自行恢复该范围，其他路径和书库继续执行。未完成范围独立于扫描任务记录持久化，删除、替换或合并失败任务不会解除输入条件，只有真实覆盖且完整完成的扫描才解除；未访问范围继续保留，根非递归完成不解除递归缺口。任务接口只读投影说明等待原因与恢复入口，等待任务保持 QUEUED。新的扫描请求合并未完成范围；完整枚举才允许按原缺失策略清理。应用层 `ContinueImportTask(force=True)` 只清该资源资产的成功处理版本并合并本书待办，保留可读结果和身份；默认继续导入仍增量复用。
...
- Book 阶段直接写 Resource／Asset，成功完成必要范围后在同一本书内执行本地识别与终态；资源失败保留有效结果且 Book 不虚报成功，任务级有限退避后仍失败则保留可见失败。扫描失败须先恢复完整清单，取消不触发收尾。本地必跑，联网按自动整理设置；文件状态与元数据状态独立，联网失败保留本地结果。
...
- BookMetadata 的 importRevision、processedRevision、metadataPending、metadataState 是本地识别修订与展示投影；活动 Book 任务拥有唯一调度事实。识别提交检查本书执行版本、修订、人工编辑时间和来源优先级，不运行每轮全局就绪搜索。终态独立提交，失败保留已执行结果按有界退避或明确继续重试；重启保留已提交资源与识别结果，仅补未完成阶段或终态。
