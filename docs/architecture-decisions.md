...
- 应用移动复用现有路径更新，将原节点及子节点的路径、父节点和书库归属同步到新位置，保持 Book／Resource ID、进度和书架关联，再请求既有扫描。删除复用现有 SourceNode 删除收尾后扫描，删除最后一本书无需放宽普通扫描的空库保护；外部文件变动仍按既有扫描观察规则处理。授权、路径边界、目标冲突和执行互斥沿用既有规则。
