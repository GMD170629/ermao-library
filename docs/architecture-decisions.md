...
- HTTP 和 HTTPS 均支持域名、IP、端口和实际部署前缀，不再有额外 HTTP 选项；合法 URL、Bearer、MCP 传输的 Origin/Host 校验保留，公开地址在保存及读取时统一规范化域名大小写、IDNA、IPv6 和默认端口，Host/Origin 使用同一规范地址；入口为实际根地址的 `/api/mcp`。
