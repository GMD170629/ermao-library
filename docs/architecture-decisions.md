...
- 单文件资源按 `IMPORT_RESOURCE` 调度，sourceNodeId 必须为资源锚点、role 为空；同资源只有一个任务行。排队请求合并，运行中有效变化设置 rerunRequested，终结本次执行时保留下一次待办；仍为单消费者，不增加抢占或自动接管。图片目录与音频目录暂保留 IMPORT_ASSET。
