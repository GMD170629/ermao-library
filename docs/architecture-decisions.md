...
- `/api/mcp` 使用锁定的官方 SDK 和 Bearer；授权管理仍使用第一方会话，自动化凭证不接入其他 REST 认证。服务默认关闭，每次调用和持久文件任务关键阶段重新验证当前用户、固定 grant、部署能力与书库范围；管理员没有令牌范围绕过。
...
- 协议 2 根据本地记录和目标清单计算依赖差异，复用现有安装器。保留的依赖不进行内容扫描；仅为实际安装的 Python 包记录安装结果，不把该记录与发行身份再比对。运行启动结果决定安装成功，不用额外 import/uv check 探针代替。
- 管理员权限、官方来源、当前准备包与确认目标绑定、安装互斥、安全路径/解压/写入范围、固定资源上限、网络超时、取消、停机备份及失败保护保持。删除或覆盖不归本包所有的文件仍被禁止。安装前从已校验归档重新生成临时目录，不能使用先前已解压的旧树。文件缺失、写入/备份/实际安装失败继续停止并记录具体阶段和公开错误码。
...
- 旧安装请求和未完成标记只作为诊断，不得拒绝普通重启或完整镜像升级。固定入口获得启动锁后，持准备锁归档旧请求/标记，避免自动重放安装；保留日志、数据库备份和已记录错误。中断中的安装状态记为 `CONTAINER_RESTARTED`，不标记更新成功。
- 重启继续既有镜像同步及程序启动流程。仍按实际文件操作、迁移和服务启动结果返回，不因历史失败推断权限不足、目录损坏或服务状态。该规则取代旧文档中“失败标记阻止重启”的要求；不增加自动回滚或清空数据库行为。
