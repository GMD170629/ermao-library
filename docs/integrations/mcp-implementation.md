...
> 2026-09-22 六项能力调整：当前权限为 `system:read`、`system:manage`、`books:write`、`shelves:write`、`files:upload`、`files:modify`。本文下方 M0–M8 是历史记录，旧细分权限不再有效。基础查询固定开启，仍受账户角色与书库范围限制；系统管理者才能读取全局队列、配置和结构化日志。迁移 0028 保留凭证和任务，仅将旧授权和服务能力重置为基础查询；0029 保存永久删除的冻结方案及逐项阶段。
