...
> 2026-09-22 原文件操作简化：当前行为以 [ADR 0031](../architecture-decisions.md#自动化-mcp-接入) 的统一书库原文件操作规则为准。新增／替换／删除／移动使用系统或标准库，再复用现有身份更新、删除收尾与扫描；目录删除包含执行时全部内容。移动计划版本 3 区分整书结构不变的身份保留与完整卷册／归属变化的重新导入；下方“所有移动保留身份”“拒绝归属变化”已被取代。重新导入清理旧索引后扫描，不迁移数据库信息或带走源目录旁的元数据，预览返回双语身份策略。下方自写跨盘复制、搬运哈希、恢复副本／配额、冻结逐项删除及替换保留原件均为已被取代的历史实现。历史测试次数不证明新流程的验证结果；标准元数据写回的格式保真契约保持。

> 2026-09-22 六项能力调整：当前权限为 `system:read`、`system:manage`、`books:write`、`shelves:write`、`files:upload`、`files:modify`。本文下方 M0–M8 是历史记录，旧细分权限不再有效。基础查询固定开启，仍受账户角色与书库范围限制；系统管理者才能读取全局队列、配置和结构化日志。迁移 0028 保留凭证和任务，仅将旧授权和服务能力重置为基础查询；0029 保存永久删除方案及逐项阶段；新方案通过 JSON 执行版本区分现行语义。

> 2026-09-22 授权机制调整：当前实现以 ADR 0031 的最新补充为准。下文 M0–M8 为原交付过程记录，其中固定范围、一次性令牌和 HTTP opt-in 已由动态／指定范围、本人加密令牌取回及 HTTP/HTTPS 同等支持替代。设置页使用三个 Tab，配置按钮位于每条授权行。配置步骤与密钥持久化要求见 [MCP 配置说明](../../examples/mcp/README.md)。


当前行为以 ADR 0031 及用户后续确认的附件上传要求为准；[v2 执行计划](../plans/mcp-integration-v2.md)保留初始范围。本文记录代码证据和阶段进度。
...
实现 patch、fill_missing、单独 clear_fields 和 override_fields；人工空值不填回，保护覆盖需要 books:write、具体字段与修订版本，覆盖后保留保护。标签更新统一要求 books:write；独立标签工具保留精确 tags 覆盖和逐书版本。
...
read_file_metadata 明确 embedded／sidecar，报告真实来源相对文件、格式和文件版本。多个有效伴随文件返回歧义，必须指定关联候选；不执行来源优先级合并、封面提取、数据库刷新或写回。读取中源文件版本改变会拒绝返回。PDF embedded 逐字段合并 Info 与有界 XMP 标准字段，XMP 有效值优先，损坏时保留 Info；与书库导入及写回计划“修改前”预览共用读取规则。writable_fields 仍按格式写回预检返回，不把可读取字段宣传成可写字段。
...
- Web 设置新增「自动化授权」，沿用现有设置导航、会话与双语机制。管理员控制服务和公共 URL；用户选择固定书库、权限、有效期及文件写回子权限。令牌仅在本次创建后内存显示，刷新/关闭清除，模板默认占位符，显式勾选才导出凭证。
- 新增 Cookie 会话下的本人文件任务记录和取消入口，仍按当前账户/书库权限过滤。撤销令牌或关闭 MCP 不妨碍本人取消已有任务；没有新增会话执行文件操作的入口。展示逐项相对路径、实际阶段与恢复提醒，不把受理当作成功。
- 前端类型通过既有 OpenAPI 生成器生成 automation 契约。模板与 5 个 Python 示例在 `examples/mcp`，固定官方 SDK；默认只读/预览，显式 execute/request-id 才写，文件重试保留原 plan-id。
- 9 项授权 HTTP/任务管理检查通过；五个独立 CLI 在真实 HTTP MCP 与临时书库运行，覆盖默认无写、创建书架、系统更新、文件移动和 OPF 写回，1 项端到端检查通过。桌面与窄屏 Chrome 共 6 项页面测试通过，一次性令牌/显式导出/撤销/任务取消/英文普通账户均覆盖，并查看实际截图。
- Web 类型检查、定向 ESLint、2276 条双语消息检查通过。Node 为当前机器 22.15.0（仓库声明 22.23.1），未擅自更换工具链。测试开发服务器使用项目现有 Webpack 模式；初始 Turbopack 启动未稳定完成页面编译，未以该失败替代页面验收。
- MCP 桌面客户端验收仍属于 M8；目前机器有 Cursor、未发现 LM Studio，尚未宣称任何桌面客户端通过。

## M3 补齐：已保存封面引用

`get_metadata_schema` 为当前图书返回最多 50 个本地导入的不可变封面引用；`cover_ref` 适用于 Book、Resource 和目录节点。输入不能是 URL 或路径，引用须仍属于当前图书和授权书库；用户上传的可覆盖槽位不作为共享候选。清空/覆盖沿用版本与逐字段保护检查，根目录同步 Book 封面，全部复用既有数据库元数据 writer，不创建、下载或删除封面文件。路径只在应用端口与存储适配器之间传递，schema/审计只记录不透明引用。封面及相邻系统修改/刷新/副作用定向检查 16 passed，5 个相关文件局部 Mypy 通过（导入模块采用 silent，不把无关旧模块类型错误计为本批通过）。

## M8：定向验收与真实客户端边界

- 修复恢复副本定位：复用不可变任务清单和持久状态，已完成但仍保留恢复副本的节点及祖先不能再次移动。清理经验证后解除；未知受控槽位也拒绝搬运。跨盘 APFS 实验覆盖正常发布及两个真实中断恢复点。
- 修复音频读写映射：共用 ID3/FLAC/MP4 映射，多个非 ASCII 作者可读回；正确替换 ID3v2.3 UTF-16 简介而保留其他原始帧。拒绝仅有 ID3 头而没有有效 MPEG 音频的文件。
- 元数据替换即使大小不变也推进毫秒级修改时间，避免旧阅读缓存命中。真实 HTTP 流验证已打开句柄读完旧版本，旧版本新请求返回 412，新请求读取新内容。跨库移动补验阅读进度行、revision、位置及书架/资源 ID 保持。
- 恢复事务提交移入 Metadata 应用用例，装配根仅接线。11 个核心文件定向 Mypy 通过，Ruff 通过。
- 后端联合定向集 264 passed；随后增强实际读取/进度断言的 16 项检查通过。Web 桌面/窄屏 6 项通过，类型检查及定向 ESLint 通过。最新双语检查 2279 条。完整命令和覆盖边界见验收记录。
...
## 2026-09-22：对话附件上传与封面更新

新增三个纯 MCP 上传工具、独立文件上传／封面权限和 0027 上传日志迁移，复用既有操作列表。Automation 管理断点、幂等与授权；Imports 使用同名拒绝的原子原件发布和现有局部扫描；Library 复用图片验证、封面发布及 Book 元数据人工保护。网页上传默认命名行为保持。Worker 在原循环有界清理过期暂存。

状态区分字节传输、文件保存、导入等待／执行与实际结果。数据库异常保留真实失败；发布后重试根据持久证明继续登记，原件与旧封面受到保护。跨上传失败独立，历史恢复状态不阻塞新上传。

纯 JSON 调用示例、客户端兼容边界及验收范围见 [附件说明](../../examples/mcp/attachment-uploads.md)。真实聊天客户端的附件字节传输未验收，不宣称任意 MCP 客户端可上传附件。本次不发布。

## 2026-09-22：三组六项能力交付

权限、服务设置、授权创建与编辑、工具发现和执行重检统一为六项。新增系统只读投影与五类配置更新，静态／智能书架更新删除，冻结清单永久删除和完整文件替换。删除先通过既有 SourceNode 清理用例处理确定删除目标，再交给导入队列刷新父目录，保留空书库保护；文件删除阶段持久化，新同名文件及清单外新增文件不删除。替换校验内容摘要与冻结源身份，保留隐藏恢复原件，发布中断按原 upload_id 继续。

迁移 0028 使用 SQLite 原生 DROP COLUMN，避免重建授权父表级联删除幂等回执。迁移测试包含历史凭证及回执；本地测试数据库及令牌密钥已备份，迁移后逐表核对原有数据与原令牌均保留，仅权限和服务允许能力重置。既有 REVIEW 写回状态未修改、未重跑。

定向验证覆盖自动化、书架、写回队列和架构检查（226 项通过），另补充最后目录删除与带历史回执的迁移检查；桌面／窄屏 24 个页面场景通过。固定官方 SDK 的真实 HTTP 调用验证系统配置与查询、智能书架、CBZ 替换后内容回读、永久删除及上传重连。类型、API 生成、双语目录、Ruff、mypy 与 Web ESLint 检查通过。本地原令牌在重置后只能发现查询工具。没有推送、发布或恢复暂停的 workflow。
