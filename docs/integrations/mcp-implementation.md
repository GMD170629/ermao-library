...
> 2026-09-22 原文件操作简化：当前行为以 [ADR 0031](../architecture-decisions.md#自动化-mcp-接入) 的统一书库原文件操作规则为准。新增／替换／删除／移动使用系统或标准库，再复用现有身份更新、删除收尾与扫描；目录删除包含执行时全部内容。下方自写跨盘复制、搬运哈希、恢复副本／配额、冻结逐项删除及替换保留原件均为已被取代的历史实现。历史测试次数不证明新流程的验证结果；标准元数据写回的格式保真契约保持。
