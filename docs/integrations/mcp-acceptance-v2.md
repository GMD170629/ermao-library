...
## 2026-09-22 授权页面与机制调整验收

当前契约以 ADR 0031 最新补充为准，后文原 M8 记录保留历史结果。本次未操作真实书库、未创建版本或标签。

- 默认授权列表、三个 URL Tab、刷新和前进后退、按需表单、动态／指定范围、管理员／普通用户的关闭提示已在 Chrome 桌面与 Pixel 7 窄屏验证。
- 每行配置默认 Codex，LM Studio／Cursor 切换、完整预览、真实剪贴板复制和下载内容已验证；历史／过期／撤销拒绝，关闭／离开 Tab 清理，延迟响应不串用令牌，浏览器存储无令牌。七个页面场景覆盖两个尺寸；已查看截图。
- 后端自动化及架构检查覆盖动态新增库、ACL 收回、旧请求固定范围、无库动态授权、冻结方案不扩目标、worker 与历史访问重新授权。HTTP 域名经过真实 Node 部署前缀网关及官方 SDK 调用通过，IP 直连通过；非法 URL、Host／Origin 和 Cookie/Bearer 边界保持。
- AES-GCM 用户／授权绑定、篡改拒绝、并发原子建钥、密钥损坏／丢失不替换、数据库与审计无明文、本人会话取回、过期／撤销、历史摘要兼容、服务关闭拒绝新建但允许取回已验证。
- 0026 保留已有授权的 selected 模式、ID 和摘要；升级／降级重入、新库和既有账户无自动授权检查通过。相关领域、事务与响应契约检查通过；Mypy（37 个模块）、改动文件 Ruff、Web 类型、局部 ESLint 和 2,287 条双语目录检查通过。

Codex CLI 在新建临时目录和独立数据库中通过实际 MCP 调用完成验收，退出码 0。令牌创建后另加 `added-after-grant`，客户端 `list_libraries` 正确发现该库；随后仅操作指定测试图书 `allowed`：

| 项目 | 实际结果 |
| --- | --- |
| 系统元数据 | 标题改为 `Codex acceptance title`，读回一致。 |
| 文件移动 | 任务 `37fbc6459aca4b84bff89d815eb26b38` 为 COMPLETED；源节点移动至 `codex-organized`，图书／节点 ID 不变。 |
| OPF 写回 | 任务 `81df007d21904c18bd80251381ac6c02` 目标 COMPLETED；仅读写确认的标题字段，文件元数据读回一致。 |
| 个人书架 | 创建静态书架并添加 `allowed`，读回成员数 1。 |

LM Studio／Cursor 本次验证配置内容，未运行这两个桌面客户端；不据此宣称本地模型推理已验收。
