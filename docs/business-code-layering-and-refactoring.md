...
以在线阅读为例，权威约束见 `AGENTS.md` 的 **Online Reading and Offline Download Separation**：

- 在线阅读按章节／资源／页面／有界字节范围取内容，不得要求、启动或等待整文件下载，也不得隐藏在 bootstrap、预热或后台任务里；
- 离线下载由用户显式发起，仅 Download Center 拥有完整下载流程；原文件保留不等于客户端必须先拿到整文件；
- 某格式当前做不到在线读取，应明确报告能力缺口，不得通过隐式整文件 fallback 假装支持；
- “网络流式传输”“增量解析”“按需渲染”分别验收。SDK 的真实调用语义必须查证；整响应进内存后再分块写盘不是网络流式传输；
- 用慢速／未结束响应验证首批数据可以提前消费、内存有界、正文不依赖整文件完成；纯 mock 字节数组和已缓存重开不构成流式证据。逐格式、逐平台标明已验证和待验证路径。
