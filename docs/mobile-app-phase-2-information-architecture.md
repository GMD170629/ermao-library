...
- 作品详情使用连续详情流：身份与主动作之后依次展示简介、目录版本选择、横向卷册轨道、当前卷册格式/媒介与元数据，以及适用时的章节目录；不再使用“简介 / 版本”一级切换。
- Version control 只显示服务器返回的真实目录版本；单版本时隐藏。volume 选择只更新当前页面的选中卷册、主动作、章节和卷册元数据，不增加导航历史；媒介种类是当前卷册属性，不作为版本 identity。
