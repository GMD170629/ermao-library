...
flowchart LR
    ...
    Detail --> Reader
    Detail --> Playing
    Reader --> Local["本地精确 progress 事务"]
    Local --> Sync["Reader v4 单飞 best-effort PUT"]
    Sync --> Home
```
...
- 下级目录页没有阅读主 CTA；图书根页的阅读区使用服务端明确的继续阅读卷册，资源详情页的主 CTA 只打开该页的 resourceId。资源封面点击推进资源详情，不修改父页面的按钮目标。
...
- 停止旧服务器音频；
- 暂停旧服务器下载；
- 对 Reader v4 progress 做一次有界本地保存/上传，并检查音频/书签持久队列。
...
- 登录成功切换 active namespace 前若存在音频/书签持久队列，仍使用切换 Dialog。
...
下级目录内容页采用 [图书内容导航契约](mobile-book-content-navigation.md) 的精简浏览布局：系统标题／返回／更多菜单 → 数量、排序与视图控制 → 面包屑 → 下级封面节点与分页，没有身份大封面、简介、进度或快捷动作行。仅点击图书进入的目录型根页恢复图书身份、当前卷册阅读区、下载状态／阅读状态／加入／更多和图书简介，下方继续同一内容浏览区。操作栏按当前页面对象作用于图书或卷册，只有主阅读按钮单独解析续读位置。阅读区只展示明确目标卷册的名称和进度，不显示混合进度；无进度时只展示开始按钮。图书与卷册详情复用同一原生操作栏，不新增导航层。
...
- 目录目的地在详情栏直接显示浏览控制、面包屑和封面列表，不保留空的身份栏；主从导航和返回上下文不变。下述身份／主 CTA 双栏仅适用于资源详情。
