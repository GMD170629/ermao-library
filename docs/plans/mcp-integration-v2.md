...
> 后续决策（2026-09-22）：本计划中固定授权范围、令牌仅显示一次和额外 HTTP 授权要求已被用户的新计划取代。原文件新增、替换、删除、移动也已改为系统／标准库操作后复用现有书库流程；本计划中的自写跨盘复制、搬运哈希、恢复副本及其配额要求不再适用于这些操作，目录删除按执行时全部内容处理。标准格式元数据写回的格式保真要求保留。当前契约见 [ADR 0031](../architecture-decisions.md#自动化-mcp-接入)，使用说明见 [配置文档](../../examples/mcp/README.md)。
