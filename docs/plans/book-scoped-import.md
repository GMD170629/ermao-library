...
## 当前决策：单次执行（2026-09-24）

用户本次明确要求以 `develop@e40ded8a51dfa0a4bf6510e6330d5efc274ccc5f` 为参考基线，将扫描与导入改为“单次任务、两个终态、重新导入独立执行”。这一要求取代下方旧 M0–M7／R1–R5 中关于自动重试、延期重新排队、完成意图补写、失败任务自动恢复和借重新排队分片的全部规则。下方内容仅作历史背景与旧测量记录，不得作为实现或测试断言的依据。

- 每个已接受的新请求有独立 task_id 和本次工作范围；同一传输请求可幂等去重。旧任务只供查看或定位重新导入目标，新请求不得改写旧任务状态、版本或资源结果。状态只走 QUEUED → RUNNING → SUCCEEDED／FAILED，重启将孤立 RUNNING 标为 WORKER_INTERRUPTED 的 FAILED，绝不重放业务。
- 单消费者按队列顺序领取；资源和目录成员在同一 RUNNING 执行内分页与短事务提交，失败资源安全隔离后继续后续资源，必要资源或阶段失败则本次 FAILED。扫描完整且有有效资源时仍完成本地元数据收尾；本次任务结果只计算本次工作范围，不把其他卷历史失败算进来。
- 发现任务登记独立 Book 工作，不建立父扫描与 Book 的终态依赖。扫描缺口用于安全和诊断，不参与普通领取；未完整枚举不得清理未见数据。Book 对本次必要的目录输入自行完成枚举。文件操作互斥、路径边界、身份、人工元数据和 Reader 引用继续保护。
- 0030–0035 迁移不改写；0036 解除单 Book／单扫描唯一约束，0037 收敛旧尝试并保存真实未执行 pending 请求，0038 记录本轮扫描已持久化精确缺口这一事实。旧兼容列不再参与运行期调度。
- A：独立任务身份、入口与迁移；B：单次 worker、失败收尾与重启；C：扫描完整性、范围内结果与旧门禁删除；D：真实集成验收、双语界面与本决策。每批定向测试后单独提交；不自动发版。
...
本地验收环境为 `Darwin 25.6.0 arm64`、Python `3.11.15`、锁定的依赖及临时 SQLite 库；分支基线为 `39a26298c444b99b0c77a851fa012d4bcb2aeba0`。隔离候选在 `apps/api-python` 使用同一锁定虚拟环境，设置 `ERMAO_CHAPTER_CORE_LIBRARY` 指向已构建且源码未改动的原生章节库后运行全量 `pytest -q`。首次未设置该路径的运行有 30 项章节库加载失败、2529 项通过；补齐环境后同一候选全量通过。Web 已运行 `pnpm --filter @shuku/web lint`、`typecheck`、`test`、`i18n:check` 和 `build`。规模和升级探针的复跑命令及样例分布见 M6。fnOS 模板预检在本机失败于安装脚本测试：目标 Linux 脚本调用 GNU `realpath -e`，macOS 的 `realpath` 不支持此参数；改用规范临时目录后仍同样失败，需在 Linux 发布环境验证。标签专属双架构正式产物门禁、iOS 真机、真实 NAS／用户库验证仍未执行；不能把本地硬链接探针继承为这些结果。迁移回退必须先停止写入，恢复匹配的旧应用、数据库和封面／存储备份；恢复旧备份后，升级期间新增数据可能丢失。

PR #27 的 Linux CI 结果：fnOS／Web 模板与质量门禁 `pass`（3 分 47 秒），公共 Reader／移动源码契约和移动后端契约 `pass`；单架构候选镜像在 `Verify real server startup fault boundaries` 中构建，并运行 `--fault-boundaries`、`--update-boundaries`，job `pass`（9 分 20 秒）。检查运行见 https://github.com/GMD170629/ermao-library/actions/runs/35860919078。该 PR 为草稿，不等于正式 tag 的双架构构建、不可变产物核验或发布。

### M7 证据核对

| 项目 | 当前证据与判定 |
| --- | --- |
| A 接线与所有权 | 静态接线如上；生产 Worker 只领取 Book／发现任务，旧类型只作迁移及历史兼容。通过。 |
| B 功能与增量 | `test_comic_pipeline_reader.py` 的混合 EPUB／PDF／TXT／图片／音频及 CBZ／ZIP 样例走真实扫描、Book Worker、详情、Reader bootstrap／manifest 与资源 URL；`test_first_page_covers.py`、`test_source_snapshot_semantics.py` 覆盖未变资源复用、OPF／引用封面变化、单资源变化及不完整枚举保护。通过本地样例；真实 NAS 行为未执行。 |
| C 规模隔离 | 三规模五次暖缓存中位、索引与 SQL／文件访问计数、十万本本地根扫描并排空、单书 129 卷与 2000 图样例见 M4／M6。冻结的领取和完成耗时目标通过；无变化单文件本书局部扫描和冷进程首次执行已单独量化；冷磁盘／冷 NAS 尚未量化。 |
| D 中断与异常 | `test_readable_resource_pipeline.py`、`test_book_import_task_queue.py`、`test_import_worker_completion_recovery.py` 与 `test_scan_failure_recovery.py` 覆盖解析期间新请求、删除／取消、部分失败、终态写失败、进程终止及缺口恢复。通过本地故障注入。 |
| E 升级 | 十万本旧库跨进程中断后重入探针通过；`test_book_task_backfill.py` 覆盖旧任务 ID 映射、授权查询与继续，`test_sqlite_database.py::test_directory_legacy_tasks_upgrade_keeps_asset_ids_and_progress` 覆盖图片／音频旧库升级后继续处理并保持资源／资产 ID、进度、书签和受保护封面。真实用户库副本未提供，未执行。 |
| F 客户端与交付 | 后端任务／Reader API 合约、Web 构建及双语检查、Linux 候选镜像启动与故障边界通过。macOS 主机当前没有 iOS 真机和 provisioning profile，不能安装运行；正式 tag 的双架构与不可变产物门禁尚未运行，未达到正式可发布判定。 |

测试记录以对应 SHA 的执行结果为准；历史资源级验收文档只用于说明旧版本，不继承其通过结论。PR 的检查还需针对最终提交重新确认。
