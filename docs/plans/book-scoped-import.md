...
| C 规模隔离 | 三规模五次暖缓存中位、索引与 SQL／文件访问计数、十万本本地根扫描并排空、单书 129 卷与 2000 图样例见 M4／M6。冻结的领取和完成耗时目标通过；无变化单文件本书局部扫描已单独量化，真正冷数据尚未量化。 |
