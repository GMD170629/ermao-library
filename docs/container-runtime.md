...
复用原 prestart 迁移及服务启动，实际 API 版本、API/网关健康、Web 响应和本次 Worker 就绪全部通过后，才清理旧准备产物、写入镜像身份并移除未完成标记。旧安装未完成标记或请求不阻止重启；固定入口在启动前持锁将它们移至同名 `.previous` 文件，保留诊断但不自动重放请求。复制、依赖安装、迁移或健康检查失败保留现场和备份，不自动回滚或重试；仅 Worker 未就绪时沿用现有故障隔离，保留健康的核心服务，但不标记同步成功。启动失败只报告实际异常，不根据失败状态推断权限或程序损坏。
...
- `installation-incomplete`：覆盖开始后的诊断标记；安装成功后清除，容器重启时归档，不作为启动准入条件。
- `install-request.json`：当前安装请求；失败时保留到重启，重启时归档，防止自动重放。

任何旧安装阶段都允许再次执行容器启动流程；未完成请求记为 `CONTAINER_RESTARTED`，已记录失败原因不覆盖，日志及备份保留。普通重启尝试启动当前程序，更换镜像继续既有同步流程；真实的文件读写、迁移或服务启动错误仍如实报告，不承诺故障程序必然启动成功。普通 SIGTERM/SIGINT 始终表示停止容器，不触发更新后重启。
...
下载失败不影响旧服务，可手动重试。安装失败按上文日志、失败标记、数据库备份和程序一致性检查处理；旧失败标记不阻止容器重启或新更新准备。
