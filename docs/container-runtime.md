...
普通重启在镜像记录有效且无未完成替换时复用 runtime，保留在线更新的版本。镜像身份变化、记录缺失或损坏时重新应用当前镜像，可能覆盖比镜像更新的在线版本；未完成替换也由完整镜像重新同步修复。旧程序身份、初始化标记、旧文件完整性不作为覆盖修复前提。

同步持有启动锁和准备锁，验证新程序来源、摘要、协议及安全路径后，写入可靠的 `installation-incomplete` 控制记录，再替换代码和依赖并运行程序。不打开或备份数据库，不递归预检旧目录写权限，不估算空间准入，不运行 `uv pip check` 或额外依赖集合扫描。实际替换、安装失败仍终止本次操作，不启动已知部分替换的程序，不自动回滚。

镜像身份在替换完成后记录，不等待业务健康；记录、日志和清理失败只输出稳定错误码及中英文提醒，不能阻止随后启动。依赖安装结果记录失败时保留未完成标记，禁止下一次增量安装使用旧记录，完整镜像同步后恢复。旧请求只归档、不重放；归档失败则本次进程禁用在线安装消费，但继续运行应用。历史状态写回、准备目录删除、就绪诊断文件写入失败均不停止服务。

启动器只负责进程监督、停止信号与子进程回收，不再执行 HTTP、版本或 Worker 就绪验收，也没有 180 秒业务启动期限。数据库迁移和配置校验由应用启动流程负责；API、Web 或网关真实退出仍传播退出状态，Worker 保持独立故障隔离。验收工具仍主动检查业务健康，不能把启动命令发起当成运行验收通过。
...
安装前在旧服务仍运行时校验包、依赖归属与安全路径，并写入可靠的执行中记录；这些控制数据失败只拒绝更新，旧服务保持运行。停机前复核限时 300 秒，等待旧进程和遗留工具退出限时 120 秒。旧业务退出后复核输入，再替换唯一 runtime 中的代码和依赖，然后调用原启动脚本。用户数据库、图书、封面、配置、密钥及固定入口不属于替换范围。

替换并发起启动后记录 `applied`，它只表示安装执行完成。启动器不再扫描依赖或等待健康验收；页面结合目标及请求摘要绑定和实际运行版本确认“目标版本已运行”。旧 `success`、`backup` 状态继续可读。更新状态不可读但本次请求的目标版本已经可访问时，页面只提示“目标版本已运行，更新记录暂不可用”；不会声称已验证摘要。页面观察超时只结束观察，可刷新继续，不终止程序、不重发安装请求。

状态与记录：

- `install-request.json`：持久安装请求；进入本次启动前的请求不重放，无法归档时禁用本次在线安装消费。
- `installation-incomplete`：在旧程序停止前建立的控制记录；实际替换中断或依赖结果记录失败时保留，禁止后续增量安装误用旧依赖记录。完整镜像同步修复后清理。
- `preparation.json` 与 `installation.log`：更新阶段和诊断，写入失败只提醒。正常阶段为 requested/checking/stopping/copying/starting/applied，旧 success/backup 保留兼容。
- `database-before-*.sqlite3`：历史备份保留，不再创建。

日志文件不可写时，依赖安装命令输出到容器标准输出／错误。单个记录或清理失败不阻止其他收尾步骤，不停止已运行服务。普通 SIGTERM/SIGINT 仍表示停止容器，不触发自动更新或重启。
...
1. 管理员点击「下载更新」后直接开始准备，不弹出下载确认框，也不停机。后台下载、验证并解压；关闭浏览器也继续执行。
2. 状态停在「更新包已准备好，等待安装」，可稍后再安装。刷新、登录、轮询、普通启动均不会自动安装。
3. 点击「立即更新」，再次确认当前版本、已准备版本及停机提示。取消会保留准备包。确认绑定版本和 SHA-256；准备包发生变化时拒绝请求，必须重新查看后确认。
...
下载失败不影响旧服务，可手动重试。安装失败按上文日志、失败标记和程序一致性检查处理；旧失败标记不阻止容器重启或新更新准备。
...
快速应用更新从既有镜像构建并安装真实候选应用包，不重建镜像；入口、约束及验收见[快速发布规则](architecture-decisions.md#快速应用更新code-only)。协议 2 在线安装行为不变。

固定容器入口随完整镜像交付。移除更新数据库备份需要更换包含此变更的完整镜像或对应安装包，仅在线替换应用代码不会改变旧入口的备份行为。
