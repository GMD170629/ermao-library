...
> 2026-08-27 书架确认稿：[行式书架目录 v2](mobile-shelves-row-layout.md) 冻结左文字右三封面、顶部搜索与三项筛选、合集二级成员列表；继续使用本文语义令牌和平台原生导航。
...
下级目录内容页是浏览型例外，按 [图书内容导航契约](mobile-book-content-navigation.md) 直接展示浏览控制、面包屑与下级封面列表；身份由系统导航标题表达，不渲染 Hero、简介、进度或快捷动作行。点击图书进入的目录型根页恢复原有 Hero、当前卷册阅读区、快捷操作和简介，下方保留内容浏览区。阅读区复用现有进度条和主按钮，只展示当前卷册进度，不显示混合阅读进度。资源详情保持原样。
