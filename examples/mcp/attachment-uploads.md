...
使用 `purpose: "replace"`，提供授权 `library_id`、`source_node_id` 和 `list_source_nodes` 返回的 `source_version`（传为 `expected_source_version`），`filename` 必须匹配原文件。其他字节传输参数、分块和完成步骤不变。需要 `files:modify`；`files:upload` 不允许覆盖。服务端验证摘要、大小、冻结源身份和当前路径，先暂存再发布并登记索引。原文件保存在受控隐藏恢复槽中；任务和恢复记录不包含文件字节。发布中断后重试同一个 upload_id，恢复不会覆盖后来出现的不同文件。

Use `purpose: "replace"` with the granted library ID, source node ID and its `source_version` as `expected_source_version`. The filename must match the original. Replacement requires `files:modify`, validates the complete byte count and SHA-256, and publishes from staging before scheduling indexing. The original is retained in a controlled hidden recovery slot. Retry the same upload ID after interruption; a different file appearing at the target is never overwritten.
