...
Readium exclusively owns typography layout, repagination and position retention
when font size or other typography preferences change. Application code only
submits preferences through the public SDK API. Do not add character/paragraph
anchor capture, temporary text selection, compensating navigation, DOM scrolling,
layout-completion polling or custom reflow/loading state for preference changes.
Tests verify preference submission and rendered settings; they must not impose
application-defined text-anchor or page-position invariants on Readium reflow.
Existing application-level workarounds are migration debt, not precedent. This
rule does not restrict explicit user navigation through the table of contents.
