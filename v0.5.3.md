...
导入流程缩短了数据库事务范围，并改进转换进度、任务恢复和并发文件处理，降低导入期间的数据库占用。部署文档也补充了数据目录、权限、备份、反向代理和受支持格式说明。
...
Import processing now keeps database transactions shorter and improves conversion progress, task recovery, and concurrent file handling. The deployment guides also clarify data directories, permissions, backups, reverse proxies, and supported formats.
