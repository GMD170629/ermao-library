...
二毛图书是一套面向个人与家庭的自托管数字书库，用于整理 NAS、家庭服务器或本地硬盘中的电子书、PDF、漫画和有声书。系统以书库根目录的文件夹结构作为图书和卷册资源的结构来源，并提供扫描导入、元数据整理、在线阅读与收听、阅读进度同步、OPDS、Kindle 发送以及数据备份。

数据库、账户、阅读进度和系统设置均保存在自己的设备上；原始读物保留在指定目录中，不依赖第三方云端托管。

- 当前版本：`1.0.2`
- [v1.0.2 发布说明](release-notes/v1.0.2.md) · [Wiki](https://github.com/GMD170629/ermao-library/wiki) · [发布与 Android 下载](https://github.com/GMD170629/ermao-library/releases)
- 支持语言：简体中文、English
- 许可证：[MIT](LICENSE)
...
- 配置多个独立书库根目录，以平铺或卷册方式组织资源；有声书通过资源识别器处理。
- 按标题、作者、媒介类型、格式、标签、系列和阅读状态搜索筛选。
- 使用自定义书架、智能书架、阅读状态和系列管理藏书。
- 自动识别书名、作者、封面和章节，支持手动编辑和元数据补全。
- 文件夹和文件的位置决定来源节点、图书、可读资源与资产；系统不会根据标题相似度或元数据自动归组、合并或移动结构。
...
- 可响应式使用 Web，或安装为 PWA。
- 可启用 OPDS 1.2 目录，通过兼容阅读器浏览、搜索和下载；OPDS 不提供第三方阅读进度同步。
- 原生 Android/iOS 客户端提供服务器连接、登录、书库、阅读与收听、下载和管理入口；具体安装包及验收状态以发布说明为准。
