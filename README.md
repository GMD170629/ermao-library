...
二毛图书是一套面向个人与家庭的自托管数字书库，用于整理 NAS、家庭服务器或本地硬盘中的电子书、PDF、漫画和有声书。它提供文件导入与自动转换、书库检索、元数据整理、在线阅读与音频播放、阅读进度同步、外部资源下载、Kindle 发送和数据备份等能力。
...
生产镜像目前面向 `linux/amd64`（x86-64），由 `main` 分支更新 `gamersgu/shuku-starship-web:prod`。复制下面的完整内容，粘贴到 NAS 的 Docker Compose 管理器中，或保存为 `compose.yaml` 后部署：
...
安装完成后访问 `http://服务器地址:3000`。首次打开会进入管理账户创建向导。
...
1. 打开系统并根据向导创建初始管理账户。
2. 进入“设置 → 书库来源与导入”，添加 `/monitor` 或其子目录作为监控文件夹。
