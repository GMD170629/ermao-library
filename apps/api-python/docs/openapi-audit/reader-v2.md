...
| `PUT /api/reader/v2/editions/{edition_id}/epub-locations`<br>实测 `/api/reader/v2/editions/audit-edition-epub/epub-locations` | `app/modules/reader/presentation/v2.py:946` | ✅ 请求参数和实际状态有文档；响应经过运行时契约处理 | ✅ HTTP 200，标准成功 envelope。用租约保存 EPUB locations 缓存 | 无（请求前后全表行数与内容摘要一致） |
| `POST /api/reader/v2/editions/{edition_id}/epub-locations/claim`<br>实测 `/api/reader/v2/editions/audit-edition-epub/epub-locations/claim` | `app/modules/reader/presentation/v2.py:900` | ✅ 请求参数和实际状态有文档；响应经过运行时契约处理 | ✅ HTTP 200，标准成功 envelope。申请 EPUB locations 生成租约 | 无（请求前后全表行数与内容摘要一致） |
