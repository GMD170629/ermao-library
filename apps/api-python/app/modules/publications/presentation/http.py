...
def _manifest(publication: NormalizedPublication) -> PublicationManifest:
    ...
    return PublicationManifest(
        metadata=PublicationMetadata(
            ...
            readingProgression=publication.reading_progression,
        ),
        links=[
            PublicationLink(
        ...
        **{
            "https://shuku.app/reader/runtime": PublicationRuntimeFingerprint(
...
def publication_manifest(
    ...
    try:
        publication = open_publication(db, settings).manifest(
            volume_id=volume_id,
            access_scope=scope,
        )
    except (
...
def publication_resource(
    ...
    try:
        ...
        raise _not_found() from error
    etag = f'"{hashlib.sha256(resource.content).hexdigest()}"'
    headers = {
    ...
    content = b"" if request.method == "HEAD" else resource.content
    headers["Content-Length"] = str(len(resource.content))
    return Response(content=content, media_type=resource.media_type, headers=headers)
