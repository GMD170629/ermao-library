...
from sqlalchemy import ColumnElement, func, inspect as sa_inspect, select
from sqlalchemy.orm import Session

from app.core.authorization import AuthorizationContext, authorization_context
from app.models.auth import User
from app.models.library import LibraryEdition, LibraryFacet, LibraryWork
...
def _distinct_edition_options(
    db: Session,
    column: Any,
) -> list[dict[str, Any]]:
    if not _table_exists(db, LibraryEdition.__tablename__):
        return []
...
def library_filter_schema(db: Session) -> dict[str, Any]:
    ...
    options["languages"] = _distinct_edition_options(db, LibraryEdition.language)
    options["formats"] = _distinct_edition_options(db, LibraryEdition.format)
    options["importStatuses"] = _distinct_edition_options(db, LibraryEdition.import_status)
    work_origins = _distinct_work_options(db, LibraryWork.origin)
    edition_origins = _distinct_edition_options(db, LibraryEdition.origin)
...
def compile_filter_rules(
    ...
    *,
    ...
    del alias, param_prefix, edition_scope_sql, edition_scope_params
