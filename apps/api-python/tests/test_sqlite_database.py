...
def test_alembic_script_directory_has_one_linear_head() -> None:
    ...
    assert script.get_heads() == ["0018_library_import_scan_gaps"]
    assert head_revision() == "0018_library_import_scan_gaps"
...
def test_fresh_baseline_contains_source_node_writeback_schema(tmp_path) -> None:
    ...
    try:
        ...
        assert _current_revision(engine) == "0018_library_import_scan_gaps"
        operation_columns = {
            column["name"]: column
