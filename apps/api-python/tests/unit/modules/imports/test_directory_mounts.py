...
def test_mount_paths_decode_kernel_escapes_and_refresh(tmp_path):
    ...
    assert (
        directory_mount_resolver(mountinfo)(PurePosixPath("/my books\\name/title"))
        == "/my books\\name"
    )
...
def test_directory_response_marks_parent_and_children_without_host_paths(tmp_path):
    ...
    node, error, status = library_directory_tree_node(
        str(tmp_path),
        mount_root_for_path=resolve,
        diagnostics=RuntimeFailureDiagnostics(logging.getLogger(__name__), "import"),
    )
    ...
    node, _, _ = library_directory_tree_node(
        str(books),
        mount_root_for_path=resolve,
        diagnostics=RuntimeFailureDiagnostics(logging.getLogger(__name__), "import"),
    )
...
def test_container_browser_lists_mounts_and_rejects_unmounted_paths(tmp_path):
    ...
    for path in (None, "/", str(books.parent)):
        node, error, status = library_directory_tree_node(
            ...
            diagnostics=RuntimeFailureDiagnostics(
                logging.getLogger(__name__), "import"
            ),
        )
        assert status == 200 and error is None
    ...
    node, _, status = library_directory_tree_node(
        None,
        mount_root_for_path=lambda path: None,
        browse_roots=(),
        diagnostics=RuntimeFailureDiagnostics(logging.getLogger(__name__), "import"),
    )
...
def test_container_detection_keeps_restricted_empty_list_when_metadata_missing(
    tmp_path, monkeypatch
):
    monkeypatch.setenv("SHUKU_CONTAINER", "1")
    assert directory_mount_resolver(tmp_path / "missing").browse_roots == ()
    assert (
        directory_mount_resolver(tmp_path / "missing", containerized=False).browse_roots
        is None
    )
...
def test_mount_browser_filters_children_that_resolve_outside_mount(tmp_path):
    ...
    def resolve(path: Path) -> str | None:
        return (
            str(books) if path.is_relative_to(books) and path.name != "system" else None
        )
