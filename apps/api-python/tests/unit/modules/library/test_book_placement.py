...
def test_flat_always_creates_book_at_resource_node() -> None:
    for relative, is_dir in (
        ...
        assert decision.resource_root_folder_relative_path is None
...
def test_volumes_nested_resource_anchors_to_root_folder() -> None:
    ...
    assert decision.resource_root_folder_relative_path == "Series"
...
def test_volumes_root_directory_resource_anchors_to_itself_as_folder() -> None:
    ...
    assert decision.create_new_book_at_source_node is False
    assert decision.resource_root_folder_relative_path == "Audiobook"


def test_empty_book_on_root_directory_resource_discovery() -> None:
    assert (
        resource_root_folder_creates_empty_book_on_discovery(
