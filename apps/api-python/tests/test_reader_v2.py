...
def test_reader_v2_openapi_exposes_generated_request_and_response_contracts(client):
    ...
    progress_path = schema["paths"]["/api/reader/v2/editions/{edition_id}/progress"][
        "put"
    ]
    request_schema = progress_path["requestBody"]["content"]["application/json"][
        "schema"
    ]
    ...
    bootstrap_response = schema["paths"][
        "/api/reader/v2/editions/{edition_id}/bootstrap"
    ]["get"]["responses"]["200"]
    assert bootstrap_response["content"]["application/json"]["schema"]["$ref"].endswith(
    ...
    unit_properties = schema["components"]["schemas"]["ReaderUnitSummary"]["properties"]
    assert {"level", "navigationKey"}.issubset(unit_properties)
    assert (
        "navigationFingerprint"
        in schema["components"]["schemas"]["ReaderBootstrapData"]["properties"]
    )
    foliate_toc = schema["components"]["schemas"]["FoliateTocProgress"]["properties"]
    assert "navigationKey" in foliate_toc
    assert (
        schema["components"]["schemas"]["ReaderBookmark"]["properties"]["id"][
            "maxLength"
        ]
        == 5000
    )
    ...
    comic_preferences = schema["components"]["schemas"]["ComicPreferences"][
        "properties"
    ]
    assert set(comic_preferences["pageTurnAnimation"]["enum"]) == {"slide", "off"}
    assert (
        schema["components"]["schemas"]["ReaderPreferences"]["properties"][
            "schemaVersion"
        ]["default"]
        == 3
    )
    assert (
        schema["components"]["schemas"]["ReaderServerPreferences"]["properties"][
            "schemaVersion"
        ]["default"]
        == 3
    )
    assert (
        schema["components"]["schemas"]["ReaderBootstrapData"]["properties"][
            "schemaVersion"
        ]["default"]
        == 2
    )
    assert (
        schema["components"]["schemas"]["ReaderProgressPut"]["properties"][
            "schemaVersion"
        ]["const"]
        == 2
    )
    assert (
        schema["components"]["schemas"]["ReaderProgressRecord"]["properties"][
            "schemaVersion"
        ]["default"]
        == 2
    )
...
def test_reader_preferences_migrate_v2_documents_to_v3():
    ...
    server_preferences = ReaderServerPreferences.model_validate(
        {
            "schemaVersion": 2,
            "settings": {"schemaVersion": 2, "epub": {"pageTurnAnimation": "kindle"}},
        }
    )
...
def _login(client, db_session, email: str = "reader-v2@example.com") -> str:
    ...
    response = client.post(
        "/api/auth/login", json={"email": email, "password": "starshipnas"}
    )
...
def test_bootstrap_seeds_user_work_preferences_from_legacy_and_returns_typed_contract(
    client, db_session
):
    ...
    assert [unit["level"] for unit in data["units"]] == [0, 0]
    assert all(unit["navigationKey"].startswith("epub:") for unit in data["units"])
    assert data["pages"] == []
    ...
    assert (
        data["serverPreferences"]["settings"]["comic"]["pageTurnAnimation"] == "slide"
    )

    stored = (
        db_session.execute(
            text(
                "SELECT userId, workId, schemaVersion, preferences FROM ReaderBookPreference"
            )
        )
        .mappings()
        .one()
    ...
    db_session.execute(
        text(
            "UPDATE ReaderPreference SET settings = :settings WHERE id = 'legacy-pref'"
        ),
        {"settings": json.dumps({"theme": "black"})},
    )
    db_session.commit()
    inherited_again = client.get("/api/reader/v2/editions/edition-v2/bootstrap").json()[
        "data"
    ]
    assert (
        inherited_again["serverPreferences"]["settings"]["appearance"]["theme"]
        == "warm"
    )
...
def test_bootstrap_supports_all_native_reflowable_source_formats(client, db_session):
    ...
    for source_format in ("EPUB", "MOBI", "AZW", "AZW3", "PRC", "FB2", "TXT"):
        db_session.execute(
            text(
                "UPDATE LibraryEdition SET format = :source_format WHERE id = 'edition-v2'"
            ),
...
def test_native_bootstrap_filters_pseudo_units_and_returns_navigation_contract(
    client, db_session
):
    _reader_tables(db_session)
    _login(client, db_session)
    ...
    exact_metadata = json.dumps(
        {
            ...
            "level": 1,
            "navigationKey": "mobi:section-2",
        }
    )
    db_session.execute(
        text("UPDATE LibraryEdition SET format = 'MOBI' WHERE id = 'edition-v2'")
    )
    db_session.execute(
        text(
            "UPDATE LibraryReadingUnit SET href = 'mobi-section:0' WHERE id = 'unit-v2-0'"
        )
    )
    db_session.execute(
        text(
            "UPDATE LibraryReadingUnit SET href = 'filepos:0000015808', metadataJson = :metadata "
            "WHERE id = 'unit-v2-1'"
        ),
    ...
    db_session.execute(
        text(
            ...
            "VALUES ('navigation-v2', 'edition-v2', 'reflowable_source', :raw_json, "
            "'2026-07-10T01:00:00', '2026-07-10T01:00:00')"
        ),
        {
            "raw_json": json.dumps(
                {"navigationFingerprint": "navigation-fingerprint-v2"}
            )
        },
    )
    db_session.commit()
    ...
    assert data["navigationFingerprint"] == "navigation-fingerprint-v2"
    assert data["units"] == [
        {
            "id": "unit-v2-1",
            "index": 1,
            "title": "第二章",
            "href": "filepos:0000015808",
            "level": 1,
            "navigationKey": "mobi:section-2",
            "fileId": "file-v2",
            "startMs": None,
...
def test_bootstrap_normalizes_and_rewrites_stored_v2_preferences(client, db_session):
    ...
    stored = (
        db_session.execute(
            text(
                "SELECT schemaVersion, preferences, updatedAt FROM ReaderBookPreference WHERE id = :preference_id"
            ),
            {"preference_id": "legacy-book-pref"},
        )
        .mappings()
...
def test_bootstrap_preserves_valid_fields_in_partially_invalid_stored_preferences(
    client, db_session
):
    _reader_tables(db_session)
    user_id = _login(client, db_session)
...
def test_bootstrap_returns_each_edition_latest_progress_for_version_navigation(
    client, db_session
):
    _reader_tables(db_session)
    user_id = _login(client, db_session)
...
def test_progress_v2_validates_fingerprint_and_writes_v2_and_legacy_projections(
    client, db_session
):
    _reader_tables(db_session)
    user_id = _login(client, db_session)
        ...
        "location": {
            ...
            "foliate": {
                "toc": {
                    ...
                    "title": "Chapter 2",
                    "href": "two.xhtml",
                    "navigationKey": "epub:chapter-2",
                },
                "navigationFingerprint": "navigation-current",
                "section": {"current": 1, "total": 3},
                "location": {"current": 40, "next": 42, "total": 100},
                "remainingSeconds": {"section": 180.5, "total": 720.25},
            },
        },
        "percent": 50,
    }
    ...
    row = (
        db_session.execute(text("SELECT * FROM LibraryReadingProgress"))
        .mappings()
        .one()
    )
    ...
    assert extra["chapterIndex"] == 1
    assert extra["navigationKey"] == "epub:chapter-2"
    assert extra["navigationFingerprint"] == "navigation-current"
    assert extra["locationCurrent"] == 40
    assert extra["remainingTotalSeconds"] == 720.25
    ...
    assert (
        mismatched.json()["error"]["details"]["expectedContentFingerprint"]
        == fingerprint
    )
    assert (
        db_session.execute(
            text("SELECT mutationId FROM LibraryReadingProgress")
        ).scalar()
        == "mutation-1"
    )
    ...
    persisted = (
        db_session.execute(
            text("SELECT mutationId, percent FROM LibraryReadingProgress")
        )
        .mappings()
        .one()
    )
...
def test_reading_status_is_user_scoped_and_finishes_only_on_last_volume(
    client, db_session
):
    _reader_tables(db_session)
    user_id = _login(client, db_session)
    _epub_fixture(db_session)
    db_session.execute(
        text("UPDATE LibraryWork SET status = 'UNREAD' WHERE id = 'work-v2'")
    )
    db_session.execute(
        text(
            """INSERT INTO LibraryVolume (
    ...
    assert (
        db_session.execute(
            text("SELECT status FROM LibraryWork WHERE id = 'work-v2'")
        ).scalar_one()
        == "UNREAD"
    )
    assert (
        db_session.execute(
            text(
                "SELECT status FROM LibraryConsumptionState WHERE userId = :user_id AND workId = 'work-v2' AND mediaKind = 'EBOOK'"
            ),
            {"user_id": user_id},
        ).scalar_one()
        == "READING"
    ...
    final = client.get(
        "/api/reader/v2/editions/edition-v2/bootstrap?volume=volume-v2-final"
    ).json()["data"]
...
def test_progress_cursor_survives_other_client_overwriting_progress_projection(
    client, db_session
):
    _reader_tables(db_session)
    user_id = _login(client, db_session)
    ...
    assert (
        db_session.execute(text("SELECT clientId FROM LibraryReadingProgress")).scalar()
        == "client-b"
    )
    ...
    projection = (
        db_session.execute(
            text("SELECT clientId, mutationId, percent FROM LibraryReadingProgress")
        )
        .mappings()
        .one()
    )
    ...
    cursors = (
        db_session.execute(
            text(
                "SELECT clientId, highWater FROM ReaderProgressCursor ORDER BY clientId"
            )
        )
        .mappings()
        .all()
    ...
    assert (
        db_session.execute(
            text(
                "SELECT highWater FROM ReaderProgressCursor WHERE clientId = 'client-a'"
            )
        ).scalar()
        == 8
    )
...
def test_progress_cursor_atomically_rejects_concurrent_duplicate_sequence(tmp_path):
    ...
    try:
        ...
        with factory() as db:
            cursor = (
                db.execute(
                    text("SELECT highWater, lastMutationId FROM ReaderProgressCursor")
                )
                .mappings()
                .one()
            )
            ...
            assert (
                ClaimClientSequence(SqlAlchemyReaderProgressCursor(db)).execute(
                    ClaimClientSequenceCommand(
                        user_id="user-1",
                        work_id="work-1",
                        client_id="client-1",
            ...
            db.rollback()
            assert (
                db.execute(text("SELECT highWater FROM ReaderProgressCursor")).scalar()
                == 11
            )
    finally:
        engine.dispose()
...
def test_progress_cursor_lazily_backfills_visible_legacy_client_sequence(
    client, db_session
):
    _reader_tables(db_session)
    user_id = _login(client, db_session)
    ...
    cursor = db_session.execute(
        text(
            "SELECT highWater FROM ReaderProgressCursor WHERE clientId = 'legacy-client'"
        )
    ).scalar()
    assert cursor == 4
    progress = (
        db_session.execute(
            text("SELECT mutationId, position FROM LibraryReadingProgress")
        )
        .mappings()
        .one()
    )
...
def test_all_reader_v1_preferences_bootstrap_and_progress_protocols_are_gone(
    client, db_session
):
    _reader_tables(db_session)
    ...
    retired_calls = [
        (
            ...
            None,
            "Reader V2 stores per-work client preferences locally",
        ),
        (
            client.put,
            "/api/reader/preferences",
        ...
        (
            client.get,
            "/api/reader/edition-v2/bootstrap",
            None,
            "/api/reader/v2/editions/edition-v2/bootstrap",
        ),
        (
            client.get,
            "/api/editions/edition-v2/progress",
            None,
            "/api/reader/v2/editions/edition-v2/progress",
        ),
        (
...
def test_progress_v2_rejects_wrong_location_type_before_writing(client, db_session):
    ...
    assert (
        db_session.execute(text("SELECT COUNT(*) FROM LibraryReadingProgress")).scalar()
        == 0
    )


def test_progress_v2_rejects_stale_tab_user_assertion_before_writing(
    client, db_session
):
    _reader_tables(db_session)
    _login(client, db_session)
...
def test_epub_bootstrap_discards_resume_location_after_fingerprint_change(
    client, db_session
):
    _reader_tables(db_session)
    user_id = _login(client, db_session)
...
def test_comic_bootstrap_reuses_lazy_archive_index_and_returns_one_based_pages(
    ...
    assert (
        db_session.execute(
            text(
                "SELECT COUNT(*) FROM LibraryReadingUnit WHERE volumeId = 'comic-volume-v2'"
            )
        ).scalar()
        == 2
    )
    ...
    assert (
        mismatched_location_volume.json()["error"]["message"]
        == "漫画位置的 volumeId 与进度目标不匹配"
    )
    ...
    stored_location = db_session.execute(
        text(
            "SELECT locationJson FROM LibraryReadingProgress WHERE editionId = 'comic-edition-v2'"
        )
    ).scalar_one()
