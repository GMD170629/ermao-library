    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            AdministrativeStateView(state: state, retry: load) { jobs in
                ...
                }.listStyle(.plain).administrativeListSurface().overlay { if filtered(jobs).isEmpty { AdministrativeEmptyView(title: copy[.empty], systemImage: "sparkles") } }
            }
        }
        .navigationTitle(copy[.organizeTitle]).navigationBarTitleDisplayMode(.inline)
        .toolbar { ToolbarItem(placement: .topBarTrailing) { Menu { Button(copy[.candidateTitle]) { navigate(.organizeCandidates) }; Button(copy[.organizeRuns]) { navigate(.organizeRuns) }; Button(copy[.recognitionPolicyTitle]) { navigate(.recognitionPolicy) } } label: { Image(systemName: "ellipsis") } } }
...
struct OrganizeRunsView: View {
    ...
    var body: some View {
        AdministrativeStateView(state: state, retry: load) { runs in
            List(runs) { run in
                VStack(alignment: .leading, spacing: .space1) {
                    HStack { Text(run.status); Spacer(); if let started = run.startedAt { Text(started.administrativeFormatted(locale: copy.locale)).font(.caption).foregroundStyle(theme.textSecondary) } }
            ...
            }.administrativeListSurface().overlay { if runs.isEmpty { AdministrativeEmptyView(title: copy[.empty], systemImage: "clock.arrow.circlepath") } }
...
struct RecognitionCandidatesView: View {
    ...
    var body: some View {
        AdministrativeStateView(state: state, retry: load) { candidates in
            ...
            }.administrativeListSurface().overlay { if candidates.isEmpty { AdministrativeEmptyView(title: copy[.empty], systemImage: "sparkles") } }
        }
    ...
    var body: some View {
        AdministrativeStateView(state: state, retry: load) { _ in
            if let binding = Binding($policy) {
                Form {
                    Section { Toggle(copy[.scheduledRecognition], isOn: binding.scheduled); Picker(copy[.schedule], selection: binding.schedule) { ForEach(RecognitionSchedule.allCases, id: \.self) { Text(schedule($0)).tag($0) } }; Toggle(copy[.runAfterImport], isOn: binding.runAfterImport) }
                    Section("OPF") { Toggle(copy[.persistOPF], isOn: binding.persistToOPF); LabeledContent(copy[.opfQueue], value: "\(binding.wrappedValue.opfQueueCompleted) / \(binding.wrappedValue.opfQueueTotal)"); if binding.wrappedValue.opfQueueTotal > 0 { ProgressView(value: Double(binding.wrappedValue.opfQueueCompleted), total: Double(binding.wrappedValue.opfQueueTotal)).tint(theme.brandAccent) } }
                    Section(copy[.metadataPriority]) { Toggle(copy[.localMetadataFirst], isOn: binding.localMetadataFirst); ForEach(Array(binding.wrappedValue.priorities.enumerated()), id: \.element) { index, item in HStack { Text("\(index + 1)"); Text(priority(item)); Spacer(); Button { movePriority(index, -1) } label: { Image(systemName: "chevron.up") }.disabled(index == 0); Button { movePriority(index, 1) } label: { Image(systemName: "chevron.down") }.disabled(index == binding.wrappedValue.priorities.count - 1) } } }
                    Section(copy[.recognitionScope]) { Toggle(copy[.recognizeUnmatched], isOn: binding.recognizeUnmatched); Toggle(copy[.recognizeIncomplete], isOn: binding.recognizeMissingAuthorOrCover); LabeledContent(copy[.eligibleCount], value: "\(binding.wrappedValue.eligibleCount)"); if let next = binding.wrappedValue.nextRunAt { LabeledContent(copy[.nextRun], value: next.administrativeFormatted(locale: copy.locale)) } }
                }.administrativeListSurface().safeAreaInset(edge: .bottom) { AdministrativeBottomAction(title: copy[.save], working: store.operationInFlight == "save-recognition-policy") { save(binding.wrappedValue) } }
            }
        }.navigationTitle(copy[.recognitionPolicyTitle]).navigationBarTitleDisplayMode(.inline).task { await loadAsync() }.onDisappear { store.cancelPendingRequests() }.administrativeNotice(store: store)
    }
    private func schedule(_ value: RecognitionSchedule) -> String { switch value { case .hourly: "1 h"; case .sixHours: "6 h"; case .daily: "24 h"; case .manual: copy[.disabled] } }
    private func priority(_ value: MetadataPriority) -> String { switch value { case .opf: "OPF"; case .embedded: copy[.metadataSection]; case .pathAndFilename: copy[.serverPath] } }
        ...
        AdministrativeStateView(state: state, retry: load) { operations in
            List(operations) { operation in
                VStack(alignment: .leading, spacing: .spaceHalf) {
                    HStack { Text(operation.action); Spacer(); Text(operation.status).foregroundStyle(theme.textSecondary) }
            ...
            }.administrativeListSurface().overlay { if operations.isEmpty { AdministrativeEmptyView(title: copy[.empty], systemImage: "clock") } }
...
struct CategoryGovernanceView: View {
    ...
    private var mergeSheet: some View { NavigationStack { Form { Section(copy[.targetCategory]) { ForEach(categories.filter { selected.contains($0.id) }) { category in Button { targetID = category.id } label: { HStack { Image(systemName: targetID == category.id ? "largecircle.fill.circle" : "circle"); Text(category.name) } } } } }.administrativeListSurface().navigationTitle(copy[.mergeCategoriesTitle]).toolbar { ToolbarItem(placement: .cancellationAction) { Button(copy[.cancel]) { mergeShown = false } }; ToolbarItem(placement: .confirmationAction) { Button(copy[.confirmMerge], action: merge).disabled(targetID == nil) } } }.environment(\.administrativeCopy, copy).tint(theme.actionAccent) }
    private func renameSheet(_ category: GovernedCategory) -> some View { NavigationStack { Form { TextField(copy[.newCategoryName], text: $renamedValue) }.administrativeListSurface().navigationTitle(copy[.renameCategory]).toolbar { ToolbarItem(placement: .cancellationAction) { Button(copy[.cancel]) { renameCategory = nil } }; ToolbarItem(placement: .confirmationAction) { Button(copy[.confirmRename]) { rename(category) }.disabled(renamedValue.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty) } } }.environment(\.administrativeCopy, copy).tint(theme.actionAccent) }
