...
actor SharedAdministrativeSettingsClient: AdministrativeSettingsClient {
    ...
    func loadOrganizeJobs(status: OrganizeJobStatus?) async throws -> [OrganizeJob] { let wire: ErmaoShared.OrganizeJobPage = try value(try await repository.listOrganizeJobs(context: context, filter: ErmaoShared.OrganizeJobFilter(search: nil, status: map(status), page: 1, pageSize: 100))); return wire.jobs.map(map) }
    ...
    func loadRecognitionCandidates() async throws -> [RecognitionCandidate] { let wire: ErmaoShared.OrganizeCandidates = try value(try await repository.loadOrganizeCandidates(context: context)); return wire.books.map { RecognitionCandidate(id: $0.id, title: $0.title ?? "—", author: ErmaoShared.AuthorDisplay.shared.label(author: $0.author), confidence: Double($0.metadataQuality) / 100) } }
    ...
    func loadOPDSConfiguration() async throws -> OPDSConfiguration {
        let wire: ErmaoShared.OpdsSettings = try value(try await repository.loadOpdsSettings(context: context))
        var configuration = map(wire)
        configuration.publicBaseURL = AdministrativeSettingsContractKt.initialOpdsPublicBaseUrl(settings: wire, context: context)
    ...
    func loadLogs(filter: LogFilter) async throws -> LogPage { let wire: ErmaoShared.ManagementEventPage = try value(try await repository.listManagementEvents(context: context, filter: managementFilter(filter, pageSize: 100))); return map(wire) }
