...
private final class AdministrativeSettingsClientFake: AdministrativeSettingsClient, @unchecked Sendable {
    ...
    func loadDuplicateGroups() async throws -> [DuplicateGroup] { try fail(); return [] }; func loadLibraryOperations() async throws -> [LibraryOperation] { try fail(); return [] }; func mergeDuplicateWorks(_ request: MergeDuplicateRequest) async throws -> String { try fail(); return "operation" }; func undoLibraryOperation(id: String) async throws { try fail() }
    ...
    func loadSystemHealth() async throws -> SystemHealthSnapshot { try fail(); return .init(checkedAt: nil, components: [], queueRestartInProgress: false, queueRestartStatus: nil) }; func runSystemHealthCheck() async throws -> SystemHealthSnapshot { try await loadSystemHealth() }; func restartImportQueueSafely() async throws -> SystemHealthSnapshot { try await loadSystemHealth() }
