...
class WorkDetailWireTest {
    ...
    @Test
    fun preservesRealAssetPathsAndKeepsUrlOnlyPayloadsCompatible() {
        val decoded = decoder.decode(
            statusCode = 200,
            body = ASSET_PATH_COMPATIBILITY_FIXTURE,
            dataDeserializer = BookPayloadWire.serializer(),
        )
        val assets = assertIs<ApiResult.Success<BookPayloadWire>>(decoded)
            .value
            .toDomain()
            .resources
            .single()
        ...
        assertEquals("/books/中文目录/Book 1.epub", assets[0].path)
        assertEquals("/api/assets/asset-with-path/content", assets[0].url)
        assertNull(assets[1].path)
        assertEquals("/api/assets/url-only/content", assets[1].url)
    }
...
private const val ASSET_PATH_COMPATIBILITY_FIXTURE = """
    {"ok":true,"data":{"book":{
        "id":"book-paths","libraryId":"library-1","sourceNodeId":"book-paths-node","title":"Book paths",
        "visibilityState":"ACTIVE","curationState":"NONE","publicationStatus":"UNKNOWN",
        "trackingStatus":"UNTRACKED","metadataQuality":0,"coverStatus":"UNKNOWN","coverUrl":"",
        "resources":[{"id":"resource-paths","bookId":"book-paths","sourceNodeId":"resource-paths-node",
            "title":"Book paths","format":"EPUB","readerType":"reflowable","assets":[
                {"id":"asset-with-path","title":"Book 1.epub","path":"/books/中文目录/Book 1.epub","url":"/api/assets/asset-with-path/content"},
                {"id":"asset-url-only","title":"Legacy Book 1.epub","url":"/api/assets/url-only/content"}
            ]
        }]
    }}}
"""
