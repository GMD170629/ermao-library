...
3. The complete reading decision block remains directly after the hero: reading progress when available, the secondary shelf action, and the filled primary reading action. The primary label remains truthful to state (`开始阅读`, `继续阅读`, `下载后阅读`, or downloading) and is never removed merely to imitate the reference. Resume/read is the only filled accent action; shelf is clearly secondary. Both remain complete, balanced, and reachable.
