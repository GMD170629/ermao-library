...
test('book detail resource covers support selection, keyboard-accessible context management, and double-click open', async ({ page }) => {
  ...
  await shelfDialog.getByRole('button', { name: '目标书架' }).click();
  await expect(shelfDialog.getByRole('option')).toHaveText(['我的指定书架']);
  await page.keyboard.press('Escape');
  await expect(shelfDialog).toBeVisible();
  await expect(shelfDialog.getByRole('listbox')).toHaveCount(0);
  await expect(shelfDialog.getByRole('button', { name: '目标书架' })).toBeFocused();
  await page.keyboard.press('Escape');
  ...
  await topBookMenu.getByRole('menuitem', { name: '加入书架', exact: true }).click();
  await shelfDialog.getByRole('button', { name: '目标书架' }).click();
  await shelfDialog.getByRole('option', { name: '我的指定书架', exact: true }).click();
