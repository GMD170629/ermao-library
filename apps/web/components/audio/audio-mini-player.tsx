      ...
      >
        <div className="overflow-hidden rounded-[20px] border border-black/[0.08] bg-[#FFFEFC]/95 shadow-[0_14px_45px_rgba(54,43,35,0.18)] backdrop-blur-xl">
          <div className="flex min-h-[84px] items-center gap-3 px-3 py-2.5 sm:px-4">
            <Link href={pendingHref} className="flex min-w-0 flex-1 items-center gap-3 rounded-xl text-left hover:bg-black/[0.035]" aria-label={`打开《${pendingSummary.title}》图书详情`}>
              ...
              <span className="min-w-0">
                ...
                <span className={cn('mt-0.5 block truncate text-xs', player.loadError ? 'text-[#C84A32]' : 'text-[#77716B]')} role="status" aria-live="polite">
                  {player.loadError ? `加载失败：${player.loadError}` : pendingSummary.chapterTitle ? `正在准备：${pendingSummary.chapterTitle}` : '正在准备有声书…'}
                </span>
              </span>
            </Link>
            {player.loadError ? (
              <button type="button" onClick={() => void player.retry()} className="flex min-h-11 items-center gap-1.5 rounded-xl bg-[#FFF0EA] px-3 text-sm font-semibold text-[#C84A32] hover:bg-[#FFE6DC]" aria-label="重试播放">
                <RotateCcw size={17} /> <span className="hidden sm:inline">重试</span>
              </button>
            ) : <LoaderCircle size={22} className="shrink-0 animate-spin text-[#EF4D2F] motion-reduce:animate-none" aria-hidden="true" />}
            {bootstrap ? (
              <button type="button" onClick={player.cancelEditionSwitch} className="flex min-h-11 min-w-11 items-center justify-center gap-1.5 rounded-xl px-2 text-sm font-semibold text-[#625C56] hover:bg-black/[0.05]" aria-label="返回原播放">
                <ArrowLeft size={17} aria-hidden="true" /> <span className="hidden xl:inline">原播放</span>
              </button>
            ) : null}
            <button type="button" onClick={player.close} className="flex min-h-11 min-w-11 items-center justify-center rounded-xl text-[#8A847E] hover:bg-black/[0.05] hover:text-[#3C3935]" aria-label="关闭播放器">
              <X size={17} />
            </button>
          </div>
        </div>
    ...
    >
      {openPanel === 'chapters' ? (
        <section
          ...
          role="region"
          aria-label={bootstrap.chapters.length > 0 ? '章节列表' : '音轨列表'}
        >
          <header className="flex items-end justify-between gap-4 border-b border-black/[0.07] px-4 py-3.5">
            <div className="min-w-0">
              <h2 className="text-sm font-semibold text-[#292724]">{bootstrap.chapters.length > 0 ? '章节' : '音轨'}</h2>
              <p className="mt-0.5 text-xs text-[#817B74]">共 {chapterItems.length} {bootstrap.chapters.length > 0 ? '章' : '轨'} · {durationLabel}</p>
            </div>
            <button type="button" onClick={() => setOpenPanel(null)} className="flex min-h-11 min-w-11 items-center justify-center rounded-full text-[#77716B] hover:bg-black/[0.05]" aria-label="关闭章节列表">
              <X size={17} />
            </button>
          </header>
      ...
      {openPanel === 'settings' ? (
        ...
        >
          <header className="flex items-center justify-between gap-4">
            <h2 className="text-sm font-semibold text-[#292724]">播放设置</h2>
            <button type="button" onClick={() => setOpenPanel(null)} className="flex min-h-11 min-w-11 items-center justify-center rounded-full text-[#77716B] hover:bg-black/[0.05]" aria-label="关闭播放设置">
              <X size={17} />
            </button>
          </header>
          ...
          <div className="mt-3">
            <div className="flex items-center justify-between gap-3 text-sm">
              <span className="font-medium text-[#4C4843]">播放速度</span>
              <span className="font-semibold tabular-nums text-[#C8452B]">{player.playbackRate}×</span>
            </div>
            <div className="mt-2 grid grid-cols-4 gap-1.5" role="group" aria-label="选择播放速度">
          ...
          <div className="mt-3 border-t border-black/[0.07] pt-4">
            <div className="flex items-center justify-between gap-3">
              <span className="flex items-center gap-2 text-sm font-medium text-[#4C4843]"><Clock3 size={17} aria-hidden="true" />睡眠定时</span>
              <span className="text-xs tabular-nums text-[#817B74]" role="status">{sleepStatus}</span>
            </div>
            <div className="mt-2 grid grid-cols-3 gap-1.5" role="group" aria-label="设置睡眠定时">
              <button
                type="button"
      ...
      <div className="overflow-hidden rounded-[22px] border border-black/[0.07] bg-[#FFFEFC]/95 shadow-[0_16px_50px_rgba(65,48,38,0.17),inset_0_1px_0_rgba(255,255,255,0.85)] backdrop-blur-xl">
        <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] items-center gap-x-2 gap-y-1.5 px-2.5 pt-2.5 sm:px-3 md:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] md:gap-x-4 md:pt-3 lg:px-4">
          <Link href={workHref} className="flex min-h-14 min-w-0 items-center gap-2.5 rounded-[14px] p-1 text-left transition-colors duration-200 hover:bg-black/[0.035] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#EF4D2F]/60" aria-label={`打开《${bootstrap.book.title}》图书详情`}>
            <span className="relative h-12 w-12 shrink-0 overflow-hidden rounded-[10px] bg-[#EEE8E1] shadow-[0_3px_10px_rgba(62,49,40,0.16)] sm:h-[52px] sm:w-[52px]">
              <Image src={coverUrl} alt={`《${bootstrap.book.title}》封面`} fill sizes="52px" unoptimized className="object-cover" />
            </span>
            <span className="min-w-0 pr-1">
              <span className="block truncate text-sm font-semibold tracking-[-0.01em] text-[#292724]">{bootstrap.book.title}</span>
              <span
              ...
              >
                {playbackError ? `播放失败：${playbackError}` : player.chapter?.title ?? player.track?.title ?? '准备播放'}
              </span>
            </span>
          </Link>
          ...
          <div className="flex shrink-0 items-center justify-end gap-0.5 md:order-3 md:justify-self-end md:border-l md:border-black/[0.07] md:pl-2">
            <button
              ...
              aria-label={bootstrap.chapters.length > 0 ? '打开章节列表' : '打开音轨列表'}
              aria-expanded={openPanel === 'chapters'}
              aria-controls={chaptersPanelId}
            >
              <ListMusic size={18} aria-hidden="true" />
              <span className="hidden text-xs font-medium xl:inline">{bootstrap.chapters.length > 0 ? '章节' : '音轨'}</span>
            </button>
            <button
            ...
            >
              ...
              <span className="hidden text-xs font-medium xl:inline">设置</span>
              {player.sleepTimerMode ? <span className="absolute right-1.5 top-1.5 h-1.5 w-1.5 rounded-full bg-[#EF4D2F]" aria-hidden="true" /> : null}
            </button>
            <button type="button" onClick={player.close} className="flex min-h-11 min-w-11 items-center justify-center rounded-xl text-[#8A847E] transition duration-200 hover:bg-black/[0.05] hover:text-[#3C3935] active:scale-[0.97] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#EF4D2F]/50" aria-label="关闭播放器">
              <X size={17} />
            </button>
          </div>
          ...
          <div className="col-span-2 flex min-w-0 items-center justify-center gap-0.5 sm:gap-1.5 md:order-2 md:col-span-1 md:justify-self-center" data-mini-player-zone="playback">
            <button type="button" onClick={player.previousChapter} className="flex min-h-11 min-w-11 items-center justify-center rounded-full text-[#57524D] transition duration-200 hover:bg-black/[0.05] active:scale-[0.94] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#EF4D2F]/50" aria-label="上一章">
              <SkipBack size={18} fill="currentColor" />
            </button>
            <button type="button" onClick={() => player.seekBy(-player.skipBackwardSeconds)} className="flex min-h-11 min-w-11 items-center justify-center rounded-full text-xs font-semibold tabular-nums text-[#57524D] transition duration-200 hover:bg-black/[0.05] active:scale-[0.94] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#EF4D2F]/50" aria-label={`后退 ${player.skipBackwardSeconds} 秒`}>
            ...
            <button
              ...
              aria-label={playbackError ? '重试播放' : isPlaying ? '暂停' : '播放'}
            ...
            <button type="button" onClick={() => player.seekBy(player.skipForwardSeconds)} className="flex min-h-11 min-w-11 items-center justify-center rounded-full text-xs font-semibold tabular-nums text-[#57524D] transition duration-200 hover:bg-black/[0.05] active:scale-[0.94] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#EF4D2F]/50" aria-label={`前进 ${player.skipForwardSeconds} 秒`}>
              +{player.skipForwardSeconds}
            </button>
            <button type="button" onClick={player.nextChapter} className="flex min-h-11 min-w-11 items-center justify-center rounded-full text-[#57524D] transition duration-200 hover:bg-black/[0.05] active:scale-[0.94] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#EF4D2F]/50" aria-label="下一章">
        ...
        {playbackError ? (
          <div className="flex min-h-11 items-center gap-2 border-t border-[#F1D6CD] bg-[#FFF4EF] px-3 text-xs text-[#A43B26]" role="status">
            ...
            <button type="button" onClick={() => void player.retry()} className="min-h-11 shrink-0 rounded-lg px-2 font-semibold hover:bg-[#FFE6DC]">重试</button>
          </div>
        ) : null}
      </div>
    </section>
