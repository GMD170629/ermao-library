...
import { MemoryReaderV2Storage } from './memory-storage';
...
test('outbox allocates strict sequences, coalesces a slot, and compare-deletes by mutation id', async () => {
  ...
  assert.equal(toProgressPutBody(replacement).userId, 'user-1');
  assert.equal(toProgressPutBody(replacement).schemaVersion, 2);
...
test('retry blocks later sequence, then resumes in order', async () => {
  ...
  await coordinator.enqueue(progress({ editionId: 'edition-a' }));
  await coordinator.enqueue(progress({ editionId: 'edition-b' }));
...
test('coordinator never flushes another user through the active authenticated session', async () => {
  const storage = new MemoryReaderV2Storage();
  await storage.enqueueProgress(progress({ userId: 'user-a', editionId: 'edition-a' }));
  await storage.enqueueProgress(progress({ userId: 'user-b', editionId: 'edition-b' }));
  const sent: string[] = [];
  const coordinator = new ReaderProgressSyncCoordinator(storage, async (mutation) => {
