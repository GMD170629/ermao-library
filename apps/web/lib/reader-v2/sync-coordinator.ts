...
export class ReaderProgressSyncCoordinator {
  ...
  constructor(
    private readonly storage: ReaderV2Storage,
  ...
  async enqueue(input: ProgressMutationInput) {
    ...
    emitReaderDebug('info', '阅读进度已写入本地队列', {
      ...
      volumeId: mutation.volumeId ?? null,
      ...
      while (!this.lostLease) {
        ...
        try {
          ...
          await this.storage.addDiagnostic({
            ...
            code: result.outcome,
            message,
            data: { editionId: mutation.editionId, volumeId: mutation.volumeId ?? null, mutationId: mutation.mutationId }
          }, this.now());
