...
test('isolates bookmarks by user and resource', () => {
  assert.notEqual(
...
test('merges legacy resource lists in reading order and removes a single saved bookmark', () => {
  const first = toggleReaderBookmark([], {
    location: { kind: 'comic', resourceId: 'resource-1', pageIndex: 8 },
    label: '第一卷 · 第 8 页',
    percent: 15,
    createdAt: '2026-07-19T00:00:00.000Z'
  });
  const second = toggleReaderBookmark([], {
    location: { kind: 'comic', resourceId: 'resource-2', pageIndex: 3 },
    label: '第二卷 · 第 3 页',
    percent: 55,
    createdAt: '2026-07-19T01:00:00.000Z'
  });
  const merged = mergeReaderBookmarks(second, first, first);
  assert.deepEqual(merged.map((bookmark) => bookmark.id), ['comic:resource-1:8', 'comic:resource-2:3']);
