...
export function ReaderEngineRuntime({
  ...
  useEffect(() => {
    ...
    void (async () => {
      const openNextVolume = () => {
        const currentIndex = bootstrap.volumes.findIndex((volume) => volume.id === bootstrap.selectedVolume?.id);
        const nextVolume = currentIndex >= 0 ? bootstrap.volumes[currentIndex + 1] : undefined;
  ...
  const currentVolumeIndex = bootstrap.volumes.findIndex((volume) => volume.id === bootstrap.selectedVolume?.id);
  const hasNextVolume = currentVolumeIndex >= 0 && currentVolumeIndex < bootstrap.volumes.length - 1;
  ...
  const bookmarkStorageKey = useMemo(() => readerBookmarkStorageKey(
    ...
    bootstrap.selectedVolume?.id ?? null,
  ...
  useEffect(() => {
    ...
    try {
      ...
      const legacySuffix = `:${bootstrap.contentFingerprint}`;
  ...
  const jumpToBookmark = useCallback(async (bookmark: ReaderBookmark) => {
    if (bookmark.location.kind === 'comic' && bookmark.location.volumeId !== bootstrap.selectedVolume?.id) {
  ...
  const volumeNavigation: ReaderVolumeNavigation = useMemo(() => ({
    ...
    currentVolumeId: bootstrap.selectedVolume?.id ?? null,
    loading: false,
    onSelectEdition,
    onSelectVolume,
    onSelectItem: (item) => {
