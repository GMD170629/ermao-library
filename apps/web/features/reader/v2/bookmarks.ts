...
export function readerBookmarkStorageKey(userId: string, editionId: string, _volumeId: string | null, fingerprint: string) {
  return ['shuku', 'reader-bookmarks', 'v2', userId, editionId, fingerprint].join(':');
}

export function legacyReaderBookmarkStoragePrefix(userId: string, editionId: string) {
  return ['shuku', 'reader-bookmarks', 'v1', userId, editionId, ''].join(':');
}
