...
test('maps new and legacy EPUB wire anchors to the reflowable domain', () => {
  assert.deepEqual(
    wireLocationToDomain({ type: 'reflowable', format: 'mobi', cfi: 'epubcfi(/6/4)', progression: 0.4 }),
    { kind: 'reflowable', format: 'mobi', cfi: 'epubcfi(/6/4)', href: undefined, progression: 0.4 }
  );
  assert.deepEqual(
    wireLocationToDomain({ type: 'epub', cfi: 'epubcfi(/6/2)', href: 'chapter.xhtml' }),
