...
test('orders the primary actions and offers only the other media kinds', () => {
  ...
  assert.deepEqual(actions.map((item) => item.action), ['open', 'download', 'edit', 'set-media-kind', 'set-ebook', 'set-comic', 'split', 'transfer', 'delete']);
});
...
test('disables unavailable navigation without hiding management actions', () => {
  ...
  assert.equal(actions.find((item) => item.action === 'open')?.disabled, true);
