...
function metadataChecks(job: OrganizeJobView) {
  const book = job.book;
  const volumes = book.versions.flatMap((version) => version.volumes);
        ...
        <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex gap-4">
            ...
            <div data-i18n-skip className="min-w-0">
              ...
              <p className="mt-1 text-sm text-slate-500">{job.book.author} · {job.book.versions.flatMap((version) => version.volumes).map((volume) => volume.format).join(' / ')}</p>
          ...
          <dl className="mt-5 space-y-3 text-sm">
            <div><dt className="text-slate-500"><I18nText>文件路径</I18nText></dt><dd className="mt-1 break-all text-slate-800">{job.book.versions.flatMap((version) => version.volumes).flatMap((volume) => volume.files)[0]?.path || i18nAttribute("未记录")}</dd></div>
