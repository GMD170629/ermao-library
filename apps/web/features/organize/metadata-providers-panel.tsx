        ...
        <article className="max-w-3xl overflow-visible rounded-[24px] border border-[#E2DDD7] bg-white shadow-sm shadow-stone-900/[0.03]">
          ...
          <ol className="divide-y divide-[#EEEAE5] px-3" aria-label={i18nAttribute('全局数据源顺序')}>
            {providers.map((provider, index) => <li key={provider.id} className="flex min-h-[74px] items-center gap-2 py-3">
              ...
              <div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-[#2C2926]">{provider.name}</p><p className="mt-0.5 truncate text-xs text-[#918A83]">{provider.enabled ? i18nAttribute('参与自动识别') : i18nAttribute('已停用')}</p></div>
