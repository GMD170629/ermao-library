  ...
  return (
    <div className={cardClass}>
      ...
      <div className={compact ? '' : 'mt-5'}>
        ...
        {settingRow(i18nAttribute('识别成功后覆盖图书文件元数据'), i18nAttribute('开启后，数据库保存成功会在后台将最终元数据写回当前媒介版本的原始文件；写回失败不会撤销识别结果。'), <Toggle checked={policy.writeMetadataToFiles} disabled={loading} label={i18nAttribute("识别成功后覆盖图书文件元数据")} onChange={(writeMetadataToFiles) => setPolicy({ ...policy, writeMetadataToFiles })} />)}
        {settingRow(i18nAttribute('本地元数据优先'), i18nAttribute('仅控制后台远程自动识别：开启后，豆瓣、Bangumi 等远程结果只能补充本地已有元数据的空字段。'), <Toggle checked={policy.preferLocalMetadata} disabled={loading} label={i18nAttribute('本地元数据优先')} onChange={(preferLocalMetadata) => setPolicy({ ...policy, preferLocalMetadata })} />)}
      </div>
