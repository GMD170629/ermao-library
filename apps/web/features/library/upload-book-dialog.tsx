    ...
    >
      <div className="w-full max-w-xl rounded-3xl border border-black/[0.08] bg-[#FFFEFC] p-6 shadow-[0_28px_80px_rgba(47,37,31,0.22)]">
        <div className="flex items-start justify-between gap-4">
          ...
          <button type="button" disabled={uploading} onClick={closeDialog} className="inline-flex h-10 w-10 items-center justify-center rounded-full text-[#77716B] transition hover:bg-black/[0.05] disabled:opacity-50" aria-label="关闭上传">
            <X size={18} />
          </button>
        </div>
        <div className="mt-6 space-y-4">
          ...
          {selectedUploadIsAudio ? (
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="block text-sm text-slate-600">有声书名称 <span className="text-xs text-[#9A948E]">（可选）</span><input value={uploadBookTitle} onChange={(event) => setUploadBookTitle(event.target.value)} placeholder="留空时读取专辑标签" className="mt-2 h-11 w-full rounded-2xl border border-black/[0.1] bg-white px-4 text-[#393531] outline-none transition placeholder:text-[#AAA49E] focus:border-[#E8A18D] focus:ring-2 focus:ring-[#F9DED4]" /></label>
              <label className="block text-sm text-slate-600">作者 <span className="text-xs text-[#9A948E]">（可选）</span><input value={uploadBookAuthor} onChange={(event) => setUploadBookAuthor(event.target.value)} placeholder="留空时读取作者标签" className="mt-2 h-11 w-full rounded-2xl border border-black/[0.1] bg-white px-4 text-[#393531] outline-none transition placeholder:text-[#AAA49E] focus:border-[#E8A18D] focus:ring-2 focus:ring-[#F9DED4]" /></label>
              <p className="text-xs leading-5 text-[#817B75] sm:col-span-2">填写后会优先用于作品识别与现有电子书、漫画版本合并；留空则按音频标签和文件名自动识别。</p>
            </div>
          ) : null}
          <TargetDirectoryPicker value={uploadTargetPath} onChange={setUploadTargetPath} memory="upload" label="保存目录" requiredMessage="请选择保存目录" processingMode="queue" />
          <div className="flex justify-end gap-2 pt-1">
            <Button type="button" variant="secondary" disabled={uploading} onClick={closeDialog}>取消</Button>
