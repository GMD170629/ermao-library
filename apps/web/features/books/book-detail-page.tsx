  ...
  return <div className="fixed inset-0 z-[120] flex items-end justify-center bg-black/45 md:items-center md:p-6" role="dialog" aria-modal="true" aria-label={t('编辑可读资源')}>
    <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-t-3xl bg-white p-5 shadow-2xl md:rounded-3xl">
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        <label className="text-sm text-[var(--visual-color-app-text-secondary)]"><I18nText>卷标题</I18nText><input value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} className="mt-1.5 w-full rounded-xl border border-[var(--visual-color-app-divider)] px-3 py-2.5" /></label>
        <label className="text-sm text-[var(--visual-color-app-text-secondary)]"><I18nText>卷号</I18nText><input type="number" step="any" value={form.resourceIndex} onChange={(event) => setForm({ ...form, resourceIndex: event.target.value })} className="mt-1.5 w-full rounded-xl border border-[var(--visual-color-app-divider)] px-3 py-2.5 tabular-nums" /></label>
        <label className="text-sm text-[var(--visual-color-app-text-secondary)] sm:col-span-2"><I18nText>简介</I18nText><textarea rows={4} value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} className="mt-1.5 w-full resize-y rounded-xl border border-[var(--visual-color-app-divider)] px-3 py-2.5" /></label>
        <label className="text-sm text-[var(--visual-color-app-text-secondary)]"><I18nText>出版社</I18nText><input value={form.publisher} onChange={(event) => setForm({ ...form, publisher: event.target.value })} className="mt-1.5 w-full rounded-xl border border-[var(--visual-color-app-divider)] px-3 py-2.5" /></label>
        <label className="text-sm text-[var(--visual-color-app-text-secondary)]"><I18nText>出版时间</I18nText><input type="date" value={form.publishedAt} onChange={(event) => setForm({ ...form, publishedAt: event.target.value })} className="mt-1.5 w-full rounded-xl border border-[var(--visual-color-app-divider)] px-3 py-2.5" /></label>
        <label className="text-sm text-[var(--visual-color-app-text-secondary)]"><I18nText>语言</I18nText><input value={form.language} onChange={(event) => setForm({ ...form, language: event.target.value })} className="mt-1.5 w-full rounded-xl border border-[var(--visual-color-app-divider)] px-3 py-2.5" /></label>
        <label className="text-sm text-[var(--visual-color-app-text-secondary)]"><I18nText>ISBN</I18nText><input value={form.isbn} onChange={(event) => setForm({ ...form, isbn: event.target.value })} className="mt-1.5 w-full rounded-xl border border-[var(--visual-color-app-divider)] px-3 py-2.5" /></label>
        <label className="text-sm text-[var(--visual-color-app-text-secondary)]"><I18nText>标识符</I18nText><input value={form.identifier} onChange={(event) => setForm({ ...form, identifier: event.target.value })} className="mt-1.5 w-full rounded-xl border border-[var(--visual-color-app-divider)] px-3 py-2.5" /></label>
        {resource.readerType === 'audio' ? <label className="text-sm text-[var(--visual-color-app-text-secondary)] sm:col-span-2"><I18nText>朗读者</I18nText><input value={form.narrator} onChange={(event) => setForm({ ...form, narrator: event.target.value })} className="mt-1.5 w-full rounded-xl border border-[var(--visual-color-app-divider)] px-3 py-2.5" /></label> : null}
      </div>
...
export function BookDetailPage({ bookId }: { bookId: string }) {
  ...
  const displayedResourceDetail = resourceDetail && requestedLocalPresentation
    ? {
        ...
        currentChapterSortOrder: chapterPresentationSortOrder(requestedLocalPresentation.chapter),
      ...
      <div className="grid gap-6 lg:grid-cols-[150px_minmax(0,1fr)_230px]">
          ...
          {hasBookMetadata ? <div className="mt-4 flex min-w-0 flex-col items-start gap-2 text-sm sm:flex-row sm:flex-wrap sm:items-center sm:gap-x-4 sm:gap-y-2">
            {seriesName ? <span className="flex min-w-0 max-w-full items-center gap-2">
              ...
              <Link
                ...
                data-i18n-skip
                className="max-w-full truncate rounded-md font-medium text-[var(--visual-color-app-text-primary)] outline-none transition hover:text-[var(--visual-color-app-brand-accent)] hover:underline focus-visible:ring-2 focus-visible:ring-[var(--visual-color-app-focus-ring)]"
                title={seriesName}
              >{seriesName}</Link>
            </span> : null}
            {tags.length > 0 ? <div className="flex max-w-full flex-wrap items-center gap-2 sm:border-l sm:border-[var(--visual-color-app-divider)] sm:pl-4">
              {tags.map((tag) => <Link
                key={tag}
                ...
                data-i18n-skip
                className="inline-flex min-h-7 items-center rounded-lg border border-[var(--visual-color-app-divider)] bg-[var(--visual-color-app-accent-softer)] px-2.5 py-1 text-xs font-medium leading-4 text-[var(--visual-color-app-text-secondary)] outline-none transition hover:border-[var(--visual-color-app-focus-ring)] hover:bg-[var(--visual-color-app-accent-soft)] hover:text-[var(--visual-color-app-brand-accent)] focus-visible:ring-2 focus-visible:ring-[var(--visual-color-app-focus-ring)]"
              >{tag}</Link>)}
            </div> : null}
          </div> : null}
        ...
        <div className="flex flex-col items-stretch gap-2 sm:flex-row sm:items-end lg:col-start-2 xl:col-start-3 xl:flex-col xl:justify-end">
          ...
          <div className="flex w-full gap-2">
            ...
            <button type="button" onClick={(event) => openBookMenu(event.currentTarget)} onContextMenu={(event) => { event.preventDefault(); openBookMenu(event.currentTarget); }} className="ml-auto flex h-11 w-12 shrink-0 items-center justify-center rounded-xl border border-[var(--visual-color-app-divider)] bg-[color:color-mix(in_srgb,var(--visual-color-app-surface-raised)_80%,transparent)] text-[var(--visual-color-app-text-secondary)] transition hover:bg-[var(--visual-color-app-surface-raised)] hover:text-[var(--visual-color-app-text-primary)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--visual-color-app-focus-ring)]" aria-label={t('管理图书 {value0}', { value0: book.title })} aria-haspopup="menu"><Ellipsis size={20} /></button>
          </div>
        </div>
      </div>
    </section>
