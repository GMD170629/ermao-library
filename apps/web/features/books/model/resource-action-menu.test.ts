...
import { resourceActionAvailability } from './resource-action-menu';

test('returns no management actions without system-management permission', () => {
  assert.deepEqual(resourceActionAvailability({ canManage: false, readable: true, mediaKind: 'EBOOK' }), []);
});

test('orders the primary actions and offers only the other media kinds', () => {
  const actions = resourceActionAvailability({ canManage: true, readable: true, mediaKind: 'AUDIOBOOK' });
...
test('disables unavailable navigation without hiding management actions', () => {
  const actions = resourceActionAvailability({ canManage: true, readable: false, mediaKind: 'COMIC' });
