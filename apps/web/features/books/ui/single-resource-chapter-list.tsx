...
export function SingleResourceChapterList({ resource, detail, loading, error, requestedPage, onPageChange }: Props) {
  const router = useRouter();
  const { t } = useI18n();
  const isPdf = resource.format === 'PDF';
  const pageSize = detail?.page.pageSize ?? CHAPTER_DETAIL_PAGE_SIZE;
  const total = isPdf ? Math.max(0, resource.pageCount ?? detail?.page.total ?? 0) : detail?.page.total ?? 0;
  const totalPages = Math.max(1, Math.ceil(total / pageSize));
  const page = Math.min(Math.max(1, detail?.page.page ?? requestedPage), totalPages);
  const units = isPdf ? syntheticPdfPageUnits(resource, page, pageSize) : detail?.units ?? [];
  const chapterStates = isPdf
    ? units.map((unit) => pdfState(unit.pageNumber ?? unit.sortOrder + 1, detail?.currentPageNumber ?? null, detail?.progress ?? resource.progress))
  ...
  const openReader = () => router.push(`/reader/${encodeURIComponent(resource.id)}`);
    ...
    <section className="mt-6" aria-busy={loading || undefined}>
      ...
      {units.length ? <div className={cn('mt-5 divide-y divide-stone-100 border-y border-stone-100 transition-opacity', loading && 'opacity-60')}>
        {units.map((unit, index) => {
          ...
          const href = detailReaderHref(resource, unit);
          return (
            <button
              key={unit.id}
              type="button"
