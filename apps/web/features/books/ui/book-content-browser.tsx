...
type Props = Readonly<{
  ...
  onManageVersion: (entry: BookContentEntry, anchor: HTMLButtonElement) => void;
}>;

function VersionCard({ book, entry, resource, position, canManage, onOpen, onManage }: { book: BookView; entry: BookContentEntry; resource: ReadableResourceView | undefined; position: number; canManage: boolean; onOpen: () => void; onManage: (anchor: HTMLButtonElement) => void }) {
  const { t } = useI18n();
  return <article className="group relative min-w-0 rounded-2xl border border-stone-200 bg-white p-2 shadow-sm transition hover:border-orange-100 hover:shadow-md">
    <button type="button" onClick={onOpen} className="block w-full text-left focus-visible:outline-none" aria-label={t('打开版本 {value0}', { value0: entry.title })}>
...
function VolumeCard({ book, resource, position, canManage, onOpen, onManage }: { book: BookView; resource: ReadableResourceView; position: number; canManage: boolean; onOpen: () => void; onManage: (anchor: HTMLButtonElement) => void }) {
  const { t } = useI18n();
  ...
  return <article className="group relative min-w-0" data-resource-card="true">
    <button type="button" onClick={onOpen} disabled={!resource.readable} aria-label={progress.visible ? t(resource.readerType === 'audio' ? '第 {value0} 卷，收听进度 {value1}%' : '第 {value0} 卷，阅读进度 {value1}%', { value0: number, value1: progress.roundedValue }) : t('第 {value0} 卷', { value0: number })} className={cn('block w-full text-left', !resource.readable && 'cursor-not-allowed opacity-50')}>
...
export function BookContentBrowser({ book, contents, resources, loading, error, layout, sort, canManage, onLayoutChange, onSortChange, onNavigate, onPageChange, onOpenResource, onManageResource, onManageVersion }: Props) {
  const { t } = useI18n();
  const resourcesById = new Map(resources.map((resource) => [resource.id, resource]));
  const currentResource = contents?.currentResourceId ? resourcesById.get(contents.currentResourceId) : undefined;
  const versions = (contents?.entries ?? []).filter((entry) => entry.kind === 'FOLDER');
  const volumes = (contents?.entries ?? []).flatMap((entry) => entry.kind === 'FILE' && entry.resourceId ? [resourcesById.get(entry.resourceId)].filter((resource): resource is ReadableResourceView => resource !== undefined) : []);
  const visibleVolumes = currentResource ? [currentResource, ...volumes.filter((resource) => resource.id !== currentResource.id)] : volumes;
  const itemCount = versions.length + visibleVolumes.length;

  return <section className="mt-6 border-t border-stone-200 pt-8">
    <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      <h2 className="text-lg font-semibold text-stone-950"><I18nText>版本与图书卷</I18nText></h2>
      <div className="flex flex-wrap items-center gap-2" aria-label={t('图书卷显示控制')}>
    ...
    <nav className="mt-5 flex min-h-9 flex-wrap items-center gap-1 text-sm" aria-label={t('版本路径')}>
    ...
    {loading ? <div className="flex min-h-48 items-center justify-center"><span className="text-sm text-stone-500"><I18nText>正在加载图书卷</I18nText></span></div> : null}
    {!loading && itemCount === 0 ? <div className="mt-5 rounded-2xl border border-dashed border-stone-300 p-10 text-center text-sm text-stone-500"><I18nText>该版本还没有可见图书卷</I18nText></div> : null}
