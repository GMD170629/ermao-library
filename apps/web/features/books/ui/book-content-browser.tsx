...
function SourceNodeCard({ book, entry, position, canManage, onOpen, onManage }: { book: BookView; entry: BookContentEntry; position: number; canManage: boolean; onOpen: () => void; onManage: (anchor: HTMLButtonElement) => void }) {
  const { t } = useI18n();
  return <article className="group relative min-w-0 w-full sm:w-[150px]">
    <button type="button" onClick={onOpen} className="block w-full text-left focus-visible:outline-none" aria-label={t('打开来源目录 {value0}', { value0: entry.title })}>
      <div className="relative overflow-hidden rounded-xl bg-stone-100 shadow-sm transition group-hover:-translate-y-0.5 group-hover:shadow-md group-focus-within:outline group-focus-within:outline-2 group-focus-within:outline-offset-2 group-focus-within:outline-[#ff4f2a]">
        <Cover book={{ id: entry.sourceNodeId, title: entry.title, author: null, coverUrl: entry.coverUrl ?? '', gradient: book.gradient, coverStatus: '' }} className="aspect-[2/3] w-full rounded-none" size="small" />
        <span className="absolute left-2 top-2 flex h-8 w-8 items-center justify-center rounded-full bg-stone-950/55 text-[11px] font-medium tabular-nums text-white shadow-sm backdrop-blur-sm">{String(position + 1).padStart(2, '0')}</span>
      </div>
    </button>
  ...
  return <article className="group relative min-w-0 w-full sm:w-[150px]" data-resource-card="true">
    <button type="button" onClick={onOpen} disabled={!resource.readable} aria-label={progress.visible ? t(resource.readerType === 'audio' ? '可读资源 {value0}，收听进度 {value1}%' : '可读资源 {value0}，阅读进度 {value1}%', { value0: volumeNumber, value1: progress.roundedValue }) : t('可读资源 {value0}', { value0: volumeNumber })} className={cn('block w-full text-left', !resource.readable && 'cursor-not-allowed opacity-50')}>
      <div className="relative overflow-hidden rounded-xl bg-stone-100 shadow-sm transition group-hover:-translate-y-0.5 group-hover:shadow-md group-focus-visible:outline group-focus-visible:outline-2 group-focus-visible:outline-offset-2 group-focus-visible:outline-[#ff4f2a]">
        <Cover book={{ id: resource.id, title: resource.title, author: null, coverUrl: resource.coverUrl, gradient: book.gradient, coverStatus: '' }} className="aspect-[2/3] w-full rounded-none" size="small" />
        <span className="resource-cover-control absolute left-2 top-2 flex h-8 w-8 items-center justify-center rounded-full bg-stone-950/55 text-[11px] font-medium tabular-nums text-white shadow-sm backdrop-blur-sm">{formatNumber(volumeNumber, { maximumFractionDigits: 3 })}</span>
...
export function BookContentBrowser({ book, contents, resources, loading, error, layout, sort, canManage, onLayoutChange, onSortChange, onNavigate, onPageChange, onOpenResource, onManageResource, onManageSourceNode }: Props) {
  ...
  return <section className="mt-6 border-t border-stone-200 pt-8">
    ...
    {!loading && itemCount > 0 && layout === 'grid' ? <div className="mt-6 grid grid-cols-2 items-start gap-x-5 gap-y-7 sm:grid-cols-[repeat(auto-fill,150px)]">
      {sourceNodes.map((entry, index) => <SourceNodeCard key={entry.sourceNodeId} book={book} entry={entry} position={index} canManage={canManage} onOpen={() => onNavigate(entry.sourceNodeId, entry)} onManage={(anchor) => onManageSourceNode(entry, anchor)} />)}
